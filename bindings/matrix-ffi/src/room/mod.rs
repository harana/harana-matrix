...
impl Room {
    ...
    pub async fn timeline_with_configuration(
        ...
        let mut builder = ui::timeline::TimelineBuilder::new(&self.inner);
    ...
    ///       [`matrix::room::RoomMemberWithSenderInfo`] containing both the
    ...
    pub async fn get_power_levels(&self) -> Result<Arc<RoomPowerLevels>, ClientError> {
        let power_levels = self.inner.power_levels().await.map_err(matrix::Error::from)?;
...
impl From<matrix::room::knock_requests::KnockRequest> for KnockRequest {
    fn from(request: matrix::room::knock_requests::KnockRequest) -> Self {
        Self {
...
pub struct KnockRequestActions {
    inner: matrix::room::knock_requests::KnockRequest,
}
...
pub struct RoomMembersIterator {
    chunk_iterator: ChunkIterator<matrix::room::RoomMember>,
}

impl RoomMembersIterator {
    fn new(members: Vec<matrix::room::RoomMember>) -> Self {
...
mod tests {
    ...
    async fn test_state_events_are_read_out_of_the_room_state() {
        ...
        let topic = room
            .state_event("m.room.topic".to_owned(), String::new())
            .await
            .unwrap()
        ...
        assert!(
            room.state_event("m.room.topic".to_owned(), "nope".to_owned()).await.unwrap().is_none()
        );
        assert!(
            room.state_event("com.example.nothing".to_owned(), String::new())
                .await
                .unwrap()
                .is_none()
        );
        ...
        let members = room.state_events("m.room.member".to_owned()).await.unwrap();
        ...
        assert!(room.state_events("com.example.nothing".to_owned()).await.unwrap().is_empty());
    }
    ...
    async fn test_room_account_data_is_written_and_read_back() {
        use matrix::ruma::events::RoomAccountDataEventType;
