...
#[cfg(feature = "experimental-search")]
use matrix::search_index::SearchIndexStoreKind;
use matrix::{
...
impl ClientBuilder {
        ...
        Arc::new(Self {
            ...
            encryption_settings: EncryptionSettings {
                ...
                backup_download_strategy:
                    matrix::encryption::BackupDownloadStrategy::AfterDecryptionFailure,
    ...
    /// `matrix::SqliteStoreConfig` (if the `sqlite` feature is enabled).
    /// Please check
    /// `matrix::SqliteStoreConfig::with_low_memory_config`.
    ...
    pub async fn build(self: Arc<Self>) -> Result<Arc<Client>, ClientBuildError> {
        ...
        if let Some(config) = builder.request_config {
            let mut updated_config = matrix::config::RequestConfig::default();
        ...
        if let Some(x509_sign) = builder.raw_x509_signer {
            ...
            use base::crypto::x509::X509SignatureSigningError;
            ...
            impl base::crypto::x509::RawX509Signer for X509SignImpl {
                fn sign(
                    &self,
                    message: Vec<u8>,
