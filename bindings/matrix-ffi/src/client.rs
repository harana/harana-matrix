...
use matrix::media::MediaFileHandle as SdkMediaFileHandle;
use matrix::{
...
#[cfg(feature = "experimental-x509-identity-verification")]
#[matrix_ffi_macros::export(with_foreign)]
...
impl From<matrix::TransmissionProgress> for TransmissionProgress {
    fn from(value: matrix::TransmissionProgress) -> Self {
        Self {
...
impl Client {
    ...
    ///
    /// If your app stopped the `ui::sync_service::SyncService`
    ...
    /// Login using JWT
    /// This is an implementation of the custom_login https://docs.rs/matrix/latest/matrix/matrix_auth/struct.MatrixAuth.html#method.login_custom
    ...
    pub fn sync_v2(
        ...
        let client = (*self.inner).clone();
        let sdk_settings: matrix::config::SyncSettings = settings.into();
    ...
    /// homeserver.
    pub async fn tile_server(&self) -> Option<matrix::TileServerInfo> {
    ...
    pub async fn server_vendor_info(&self) -> Result<matrix::ServerVendorInfo, ClientError> {
...
impl Client {
    ...
    fn session_inner(client: matrix::Client) -> Result<Session, ClientError> {
...
/// [`Client::restore_session_with`].
///
/// Please, see the documentation of [`matrix::store::RoomLoadSettings`] to
/// learn more.
#[derive(uniffi::Enum)]
...
#[derive(uniffi::Record)]
pub struct Session {
    // Same fields as the Session type in matrix, just simpler types
...
impl Session {
    fn new(
        ...
        match auth_api {
            ...
            AuthApi::Matrix(a) => {
                let matrix::authentication::matrix::MatrixSession {
                    meta: matrix::SessionMeta { user_id, device_id },
                    tokens: matrix::SessionTokens { access_token, refresh_token },
            ...
            AuthApi::OAuth(api) => {
                let matrix::authentication::oauth::UserSession {
                    meta: matrix::SessionMeta { user_id, device_id },
                    tokens: matrix::SessionTokens { access_token, refresh_token },
                    homeserver: _,
    ...
    fn into_tokens(self) -> matrix::SessionTokens {
...
impl TryFrom<Session> for AuthSession {
    ...
    fn try_from(value: Session) -> Result<Self, Self::Error> {
        ...
        if let Some(oauth_data_string) = oauth_data_string {
            ...
            let user_session = matrix::authentication::oauth::UserSession {
                meta: matrix::SessionMeta {
            ...
            let session = matrix::authentication::matrix::MatrixSession {
                meta: matrix::SessionMeta {
                    user_id: user_id.try_into()?,
                    device_id: device_id.into(),
                },
...
impl From<matrix::StoreSizes> for StoreSizes {
    fn from(value: matrix::StoreSizes) -> Self {
        Self {
...
pub struct HomeserverCapabilities {
    inner: matrix::HomeserverCapabilities,
}

impl HomeserverCapabilities {
    pub(crate) fn new(capabilities: matrix::HomeserverCapabilities) -> Self {
...
mod tests {
    ...
    async fn client_drop_on_non_tokio_thread_does_not_panic() {
        ...
        use matrix::{Client, config::RequestConfig};
