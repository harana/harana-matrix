...
mod sys {
    ...
    use sdk_common::executor::{JoinHandle, spawn, spawn_blocking};
    pub use tokio::runtime::EnterGuard as RuntimeGuard;

    /// A handle to the Tokio runtime the bindings run their tasks on.
    ///
    /// Tasks are spawned through [`sdk_common::executor`], so they end
...
mod sys {
    ...
    use sdk_common::executor::{JoinHandle, spawn};
