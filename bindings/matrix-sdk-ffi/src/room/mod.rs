...
impl Room {
    ...
    /// Get every state event of this room with the given type, whatever its
    /// state key, as JSON strings in no particular order.
    ///
    /// # Arguments
    ///
    /// * `event_type` - The type of the state events (e.g. `"m.room.member"` or
    ///   a custom type).
