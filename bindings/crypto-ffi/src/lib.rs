//! Uniffi based bindings for the `crypto` crate.
//!
...
async fn migrate_data(
    ...
    use crypto::{olm::PrivateCrossSigningIdentity, store::types::BackupDecryptionKey};
    ...
    let pickled_account = crypto::olm::PickledAccount {
    ...
    let account = crypto::olm::Account::from_pickle(pickled_account)?;
...
fn collect_sessions(
    ...
    for session_pickle in session_pickles {
        ...
        let pickle = crypto::olm::PickledSession {
            pickle,
    ...
    for session in group_session_pickles {
        ...
        let pickle = crypto::olm::PickledInboundGroupSession {
            pickle,
            sender_key,
            signing_key: session
                .signing_key
        ...
        let session = crypto::olm::InboundGroupSession::from_pickle(pickle)?;
...
/// Take a look at [`sdk_common::deserialized_responses::ShieldState`]
/// for more info.
#[allow(missing_docs)]
#[derive(uniffi::Enum)]
...
impl TryFrom<crypto::store::types::BackupKeys> for BackupKeys {
    type Error = ();

    fn try_from(keys: crypto::store::types::BackupKeys) -> Result<Self, Self::Error> {
        Ok(Self {
...
impl From<crypto::store::types::RoomKeyCounts> for RoomKeyCounts {
    fn from(count: crypto::store::types::RoomKeyCounts) -> Self {
        Self { total: count.total as i64, backed_up: count.backed_up as i64 }
    }
}

impl From<crypto::CrossSigningKeyExport> for CrossSigningKeyExport {
    fn from(e: crypto::CrossSigningKeyExport) -> Self {
        Self {
...
impl From<CrossSigningKeyExport> for crypto::CrossSigningKeyExport {
    fn from(e: CrossSigningKeyExport) -> Self {
        crypto::CrossSigningKeyExport {
...
impl From<crypto::CrossSigningStatus> for CrossSigningStatus {
    fn from(s: crypto::CrossSigningStatus) -> Self {
        Self {
...
fn version_info() -> VersionInfo {
    VersionInfo {
        version: crypto::VERSION.to_owned(),
        vodozemac_version: crypto::vodozemac::VERSION.to_owned(),
...
pub struct VersionInfo {
    /// The version of the crypto crate.
    pub version: String,
...
pub struct PkEncryption {
    inner: crypto::vodozemac::pk_encryption::PkEncryption,
}
...
impl PkEncryption {
    ...
    pub fn from_base64(key: &str) -> Result<Arc<Self>, DecodeError> {
        let key = vodozemac::Curve25519PublicKey::from_base64(key)
            .map_err(crypto::backups::DecodeError::PublicKey)?;
