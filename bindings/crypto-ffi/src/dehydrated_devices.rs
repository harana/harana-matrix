...
pub enum DehydrationError {
    #[error(transparent)]
    Pickle(#[from] crypto::vodozemac::DehydratedDeviceError),
    #[error(transparent)]
    LegacyPickle(#[from] crypto::vodozemac::LibolmPickleError),
    #[error(transparent)]
    MissingSigningKey(#[from] crypto::SignatureError),
    #[error(transparent)]
    Json(#[from] serde_json::Error),
    #[error(transparent)]
    Store(#[from] crypto::CryptoStoreError),
    #[error("The pickle key has an invalid length, expected 32 bytes, got {0}")]
    PickleKeyLength(usize),
}

impl From<crypto::dehydrated_devices::DehydrationError> for DehydrationError {
    fn from(value: crypto::dehydrated_devices::DehydrationError) -> Self {
        match value {
            ...
            crypto::dehydrated_devices::DehydrationError::Store(e) => Self::Store(e),
            crypto::dehydrated_devices::DehydrationError::PickleKeyLength(l) => {
