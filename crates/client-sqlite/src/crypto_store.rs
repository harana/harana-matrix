...
use client_base::cross_process_lock::CrossProcessLockGeneration;
use client_crypto::{
...
impl CryptoStore for SqliteCryptoStore {
    ...
    ) -> client_crypto::store::Result<(), Self::Error> {
    ...
    async fn is_message_known(
        &self,
        message_hash: &client_crypto::olm::OlmMessageHash,
    ) -> Result<bool> {
    ...
    async fn get_withheld_sessions_by_room_id(
        &self,
        room_id: &RoomId,
    ) -> client_crypto::store::Result<Vec<RoomKeyWithheldEntry>, Self::Error> {
        let room_id = self.encode_key("direct_withheld_info", room_id);
...
mod tests {
    ...
    fn inbound_group_session() -> client_crypto::olm::InboundGroupSession {
        use client_crypto::{
...
mod encrypted_tests {
    ...
    use client_crypto::{cryptostore_integration_tests, cryptostore_integration_tests_time};
    use tempfile::{TempDir, tempdir};
...
mod close_reopen_tests {
    ...
    use client_crypto::store::CryptoStore;
    use common_test::async_test;
    use tempfile::{TempDir, tempdir};
