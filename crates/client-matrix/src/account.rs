...
use js_int::uint;
#[cfg(feature = "experimental-element-recent-emojis")]
use client_base::recent_emojis::RecentEmojisContent;
use client_base::{
...
impl Account {
    ...
    /// ```no_run
    /// # use client_matrix::Client;
    /// # use client_matrix::ruma::{ClientSecret, uint};
    /// # use url::Url;
    /// # async {
    /// # let homeserver = Url::parse("http://localhost:8080")?;
    /// # let client = Client::new(homeserver).await?;
    ...
    /// ```no_run
    /// # use client_matrix::Client;
    /// # use client_matrix::ruma::thirdparty::Medium;
    /// # use client_matrix::ruma::api::client::account::ThirdPartyIdRemovalStatus;
    /// # use url::Url;
    /// # async {
    /// # let homeserver = Url::parse("http://localhost:8080")?;
    /// # let client = Client::new(homeserver).await?;
    ...
    /// # let account = client.account();
    /// use client_matrix::ruma::events::ignored_user_list::IgnoredUserListEventContent;
    ///
    ...
    /// # let client = Client::new("http://localhost:8080".parse()?).await?;
    /// # let account = client.account();
    /// use client_matrix::ruma::events::{ignored_user_list::IgnoredUserListEventContent, GlobalAccountDataEventType};
    ///
    ...
    /// # use client_matrix::Client;
    /// # use client_matrix::ruma::events::media_preview_config::MediaPreviews;
    /// # use url::Url;
    /// # async {
    /// # let homeserver = Url::parse("http://localhost:8080")?;
    /// # let client = Client::new(homeserver).await?;
