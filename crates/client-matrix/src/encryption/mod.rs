...
#[cfg(feature = "experimental-send-custom-to-device")]
use client_base::crypto::CollectStrategy;
use client_base::{
...
#[cfg(feature = "sqlite")]
use client_base::crypto::OlmMachineBuilder;
pub use client_base::crypto::{
...
pub enum BundleExportError {
    ...
    #[error(transparent)]
    OpenStoreError(#[from] client_sqlite::OpenStoreError),
    ...
    /// the store.
    #[error(transparent)]
    SecretExport(#[from] client_base::crypto::store::SecretsBundleExportError),
}
...
pub async fn export_secrets_bundle_from_store(
    ...
    use client_base::crypto::store::CryptoStore;

    let store = client_sqlite::SqliteCryptoStore::open(database_path, passphrase).await?;
    let account =
...
impl Client {
    ...
    pub(crate) async fn mark_request_as_sent(
        ...
        response: impl Into<client_base::crypto::types::requests::AnyIncomingResponse<'_>>,
    ) -> Result<(), client_base::Error> {
        Ok(self
            .olm_machine()
            .await
            .as_ref()
    ...
    ///     client_base::crypto::olm::CrossSigningStatus::is_published
    pub(crate) async fn mark_cross_signing_identity_as_published(
        &self,
        response: &UploadSigningKeysResponse,
    ) {
        use client_base::crypto::types::requests::AnyIncomingResponse;
    ...
    /// # use url::Url;
    /// # use client_matrix::ruma::{room_id, OwnedRoomId};
    /// use serde::{Deserialize, Serialize};
    /// use client_matrix::ruma::events::{macros::EventContent, room::EncryptedFile};
    ///
    ...
    pub(crate) async fn send_verification_request(
        ...
        use client_base::crypto::types::requests::OutgoingVerificationRequest::*;
    ...
    async fn send_outgoing_request(&self, r: OutgoingRequest) -> Result<()> {
        use client_base::crypto::types::requests::AnyOutgoingRequest;
...
impl Encryption {
    ...
    ///
    /// ```no_run
    /// use client_matrix::{Client, encryption};
    /// use url::Url;
    ///
    /// # async {
    /// let homeserver = Url::parse("http://example.com")?;
    ...
    pub async fn get_verification(&self, user_id: &UserId, flow_id: &str) -> Option<Verification> {
        ...
        olm.get_verification(user_id, flow_id).and_then(|v| match v {
            client_base::crypto::Verification::SasV1(sas) => {
                Some(SasVerification { inner: sas, client: self.client.clone() }.into())
            }
            #[cfg(feature = "qrcode")]
            client_base::crypto::Verification::QrV1(qr) => {
    ...
    ///
    /// ```no_run
    /// # use client_matrix::{Client, ruma::{device_id, user_id}};
    /// # use url::Url;
    /// # async {
    ...
    ///
    /// ```no_run
    /// # use client_matrix::{Client, ruma::user_id};
    /// # use url::Url;
    /// # async {
    /// # let alice = user_id!("@alice:example.org");
    /// # let homeserver = Url::parse("http://example.com")?;
    ...
    /// # use client_matrix::{ruma::api::client::uiaa, Client};
    /// # use url::Url;
    /// # use serde_json::json;
    /// # async {
    /// # let homeserver = Url::parse("http://example.com")?;
    ...
    ///
    /// ```no_run
    /// use client_matrix::{ruma::api::client::uiaa, encryption::CrossSigningResetAuthType};
    ///
    /// # async {
    /// # let homeserver = url::Url::parse("http://example.com")?;
    /// # let client = client_matrix::Client::new(homeserver).await?;
    ...
    ///             use client_matrix::ruma::api::client::uiaa;
    ///
    ...
    pub async fn export_room_keys(
        ...
        path: PathBuf,
        passphrase: &str,
        predicate: impl FnMut(&client_base::crypto::olm::InboundGroupSession) -> bool,
    ) -> Result<()> {
        let olm = self.client.olm_machine().await;
        let olm = olm.as_ref().ok_or(Error::NoOlmMachine)?;
        ...
        let encrypt = move || -> Result<()> {
            let export: String =
                client_base::crypto::encrypt_room_key_export(&keys, &passphrase, 500_000)?;
    ...
    pub async fn import_room_keys(
        ...
        let decrypt = move || {
            ...
            client_base::crypto::decrypt_room_key_export(file, &passphrase)
        };
...
mod tests {
    ...
    async fn test_a_rejected_key_upload_is_not_retried_forever() {
        ...
        let has_key_upload = async |machine: &client_base::crypto::OlmMachine| {
            machine
                .outgoing_requests()
                .await
                .unwrap()
    ...
    async fn test_generation_counter_invalidates_olm_machine() {
        ...
        use client_base::store::RoomLoadSettings;
        ...
        let decryption_key = client_base::crypto::store::types::BackupDecryptionKey::new();
    ...
    async fn test_a_rejected_duplicate_one_time_key_upload_is_not_retried_forever() {
        ...
        assert!(
            !outgoing.iter().any(|request| matches!(
                request.request(),
                client_base::crypto::types::requests::AnyOutgoingRequest::KeysUpload(_)
            )),
