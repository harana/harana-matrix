...
use http::StatusCode;
#[cfg(feature = "qrcode")]
use client_base::crypto::ScanError;
...
#[cfg(feature = "sqlite")]
impl From<client_sqlite::OpenStoreError> for Error {
    fn from(error: client_sqlite::OpenStoreError) -> Self {
        ClientBuildError::from(error).into()
    }
}

#[cfg(feature = "indexeddb")]
impl From<client_indexeddb::OpenStoreError> for Error {
    fn from(error: client_indexeddb::OpenStoreError) -> Self {
        ClientBuildError::from(error).into()
    }
}
...
mod tests {
    ...
    fn test_client_build_error_propagates_with_the_question_mark_operator() {
        // The point of the conversion: a function returning `client_matrix::Error` can
    ...
    fn test_open_store_error_propagates_with_the_question_mark_operator() {
        fn open() -> Result<(), client_sqlite::OpenStoreError> {
            Err(client_sqlite::OpenStoreError::MissingVersion)
        }
