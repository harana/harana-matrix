...
impl MatrixAuth {
    ...
    /// ```no_run
    /// use client_matrix::Client;
    /// # use client_matrix::utils::UrlOrQuery;
    /// # use url::Url;
    /// # let homeserver = Url::parse("https://example.com").unwrap();
    /// # let redirect_url = "http://localhost:1234";
    ...
    ///
    /// ```no_run
    /// use client_matrix::{Client, Error};
    /// use url::Url;
    /// # async {
    /// # fn get_credentials() -> (&'static str, &'static str) { ("", "") };
    /// # fn persist_session(_: Option<client_matrix::AuthSession>) {};
    ///
            ...
            Ok(res) => {
                *guard = Ok(());
                ...
                if let Some(save_session_callback) =
                    ...
                    && let Err(err) = save_session_callback(self.client.clone()).await
                {
    ...
    ///
    /// ```no_run
    /// use client_matrix::{Client, store::RoomLoadSettings};
    /// use url::Url;
    /// # async {
    ///
    /// let homeserver = Url::parse("http://example.com")?;
