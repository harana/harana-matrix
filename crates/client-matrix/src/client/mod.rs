...
pub(crate) struct ClientInner {
    ...
    /// [`client_common::cross_process_lock::CrossProcessLock`]) when
...
impl Client {
    ...
    ///
    /// The SDK provides cross-process store locks (see
    /// [`client_common::cross_process_lock::CrossProcessLock`]) when this
    ...
    /// # let mut client = Client::new(homeserver).await?;
    /// use client_matrix::ruma::{api::client::profile, owned_user_id};
    ///
    ...
    ///
    /// ```no_run
    /// use client_matrix::ruma::api::FeatureFlag;
    /// # use client_matrix::{Client, config::SyncSettings};
    /// # use url::Url;
    /// # async {
    /// # let homeserver = Url::parse("http://localhost:8080")?;
    ...
    ///
    /// ```no_run
    /// # use client_matrix::{Client, ruma::api::client::authenticated_media::get_media_preview};
    ...
    /// # Examples
    /// ```no_run
    /// # use client_matrix::{Client, config::SyncSettings, ruma::api::client::rtc::RtcTransport};
    /// # use url::Url;
    /// # async {
    /// # let homeserver = Url::parse("http://localhost:8080")?;
    /// # let mut client = Client::new(homeserver).await?;
    ...
    /// # use client_matrix::{Client, config::SyncSettings, LoopCtrl};
    /// # use url::Url;
    /// # async {
    /// # let homeserver = Url::parse("http://localhost:8080").unwrap();
    /// # let mut client = Client::new(homeserver).await.unwrap();
    ...
    /// `cross_process_lock_config`.
    ///
    /// [`CrossProcessLock::new`]: client_common::cross_process_lock::CrossProcessLock::new
...
pub(crate) mod tests {
    ...
    async fn test_threading_support_reflects_the_builder_configuration() {
        use client_base::ThreadingSupport;
