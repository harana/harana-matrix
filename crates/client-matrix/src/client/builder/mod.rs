...
use client_base::crypto::DecryptionSettings;
#[cfg(feature = "experimental-x509-identity-verification")]
use client_base::crypto::x509::{RawX509Signer, RawX509Verifier};
#[cfg(feature = "e2e-encryption")]
use client_base::crypto::{CollectStrategy, TrustRequirement};
use client_base::{
...
#[cfg(feature = "experimental-search-core")]
use client_search::backend::SearchIndexProvider;
#[cfg(feature = "sqlite")]
...
impl ClientBuilder {
    ...
    ///
    /// ```
    /// # use client_base::store::MemoryStore;
    /// # use client_common::cross_process_lock::CrossProcessLockConfig;
    /// # let custom_state_store = MemoryStore::new();
    /// use client_matrix::{Client, config::StoreConfig};
    ///
    ...
    /// [`client_common::cross_process_lock::CrossProcessLock`]). The
    ...
    #[instrument(skip_all, target = "client_matrix::client", fields(homeserver))]
    ...
    let store_config = match builder_config {
        ...
        BuilderStoreConfig::Sqlite { config, cache_path } => {
            ...
            let (state_store, event_cache_store, media_store, crypto_store) = try_join!(
                client_sqlite::SqliteStateStore::open_with_config(&config),
                client_sqlite::SqliteEventCacheStore::open_with_config(&config_with_cache_path),
                client_sqlite::SqliteMediaStore::open_with_config(&config_with_cache_path),
                client_sqlite::SqliteCryptoStore::open_with_config(&config),
            )?;
            #[cfg(not(feature = "e2e-encryption"))]
            let (state_store, event_cache_store, media_store) = try_join!(
                client_sqlite::SqliteStateStore::open_with_config(&config),
                client_sqlite::SqliteEventCacheStore::open_with_config(&config_with_cache_path),
                client_sqlite::SqliteMediaStore::open_with_config(&config),
            )?;
...
async fn build_indexeddb_store_config(
    ...
    let stores = client_indexeddb::IndexeddbStores::open(name, passphrase).await?;
...
pub enum ClientBuildError {
    ...
    #[cfg(feature = "indexeddb")]
    #[error(transparent)]
    IndexeddbStore(#[from] client_indexeddb::OpenStoreError),

    /// Error opening the sqlite store.
    #[cfg(feature = "sqlite")]
    #[error(transparent)]
    SqliteStore(#[from] client_sqlite::OpenStoreError),
...
pub(crate) mod tests {
    ...
    impl StoreProvider for TestStoreProvider {
        fn open_stores<'a>(
            &'a self,
            cross_process_lock_config: &'a CrossProcessLockConfig,
        ) -> client_common::BoxFuture<'a, Result<StoreConfig, StoreProviderError>> {
            Box::pin(async move {
