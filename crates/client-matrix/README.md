...
- [`client-base`]: A no-network-IO client state machine which can be used
  to embed a Matrix client into an existing network stack or to build a new
  Matrix client library on top.
- [`client-crypto`]: A no-network-IO encryption state machine which can be
...
- **Spawning tasks, sleeping and running blocking work.** This is described by
  [`client_common::runtime::AsyncRuntime`]. Tokio is used by default (the
  `tokio-runtime` feature); install your own with
  [`client_common::runtime::set_runtime`] before creating a [`Client`].
...
[`client-crypto`]: https://docs.rs/client-crypto/
[`client-base`]: https://docs.rs/client-base/
