...
use client_base::{RequestedRequiredStates, RoomState};
use common_test::{
...
#[async_test]
async fn test_room_preview_local_first_answers_from_local_data() {
    let (client, server) = logged_in_client_with_server().await;
    let room_id = room_id!("!room:localhost");
    ...
    // The server knows a fresher name, and it answers slowly.
    ...
    // The local preview comes back without waiting on the server.
    let (preview, refresh) =
        client.get_room_preview_local_first(room_id.into(), Vec::new()).await.unwrap();
    assert_eq!(preview.room_id, room_id);
    assert_eq!(preview.name.as_deref(), Some("Local name"));
    assert_matches!(preview.state.unwrap(), RoomState::Invited);

    // The refresh happens in the background, and saves what the server said.
    refresh.expect("a refresh was started for an invited room").await.unwrap();

    let room = client.get_room(room_id).unwrap();
    assert_eq!(room.name().as_deref(), Some("Remote name"));
    assert_eq!(room.topic().as_deref(), Some("Remote topic"));
    assert_eq!(room.joined_members_count(), 3);

    // So the next local preview is the fresher one.
    let (preview, _) =
        client.get_room_preview_local_first(room_id.into(), Vec::new()).await.unwrap();
    assert_eq!(preview.name.as_deref(), Some("Remote name"));
}

#[async_test]
async fn test_room_preview_local_first_does_not_refresh_a_joined_room() {
    let server = MatrixMockServer::new().await;
    let client = server.client_builder().build().await;
    let room_id = room_id!("!room:localhost");
    ...
    // A joined room is kept up to date by sync, so nothing is fetched: no summary
    // endpoint is mocked, and a request to it would fail the test below.
    let (preview, refresh) =
        client.get_room_preview_local_first(room_id.into(), Vec::new()).await.unwrap();

    assert_eq!(preview.room_id, room_id);
    assert_matches!(preview.state.unwrap(), RoomState::Joined);
