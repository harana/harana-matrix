...
//! # Usage
//!
//! Begin by creating a `Client`, selecting one of the type aliases from `client_ruma::http_client`
//! for the generic parameter. For the client API, there are login and registration methods
//! provided for the client:
//!
//! ```no_run
//! # async {
//! # type HttpClient = client_ruma::http_client::Dummy;
//! let homeserver_url = "https://example.com".to_owned();
//! let client =
//!     client_ruma::Client::builder().homeserver_url(homeserver_url).build::<HttpClient>().await?;
//!
...
//! # Result::<(), client_ruma::Error<_, _>>::Ok(())
//! # };
//! ```
//!
...
//! # type HttpClient = client_ruma::http_client::Dummy;
//! let homeserver_url = "https://example.com".to_owned();
//! let client = client_ruma::Client::builder()
