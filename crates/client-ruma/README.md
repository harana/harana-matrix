...
A minimal Matrix client on top of `common-ruma`. Nothing else in this workspace
depends on it; `client-matrix` has its own HTTP client.
...
- The `client-api` feature is gone. It gated `Client` on `common-ruma`'s
  client-server API types, which the vendored `common-ruma` always provides.
