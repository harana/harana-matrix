...
use common_signatures::{Ed25519KeyPair, KeyPair, PublicKeyMap};
use thiserror::Error;
use tracing::debug;
...
impl XMatrix {
    ...
    pub fn try_from_http_request<T: AsRef<[u8]>>(
        ...
        // common_signatures::sign_json, however since we would need to extract the signature from the
    ...
    pub fn verify_request<T: AsRef<[u8]>>(
        ...
        let mut request_object = Self::request_object(request, &self.origin, destination)
            .map_err(|error| common_signatures::VerificationError::Json(error.into()))?;
        ...
        Ok(common_signatures::verify_json(public_key_map, &request_object)?)
    }
}
...
pub enum XMatrixVerificationError {
    ...
    Signature(#[from] common_signatures::VerificationError),
}
