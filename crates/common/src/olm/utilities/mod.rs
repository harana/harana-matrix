...
const STANDARD_NO_PAD: GeneralPurpose = GeneralPurpose::new(
    ...
    general_purpose::NO_PAD
        .with_decode_padding_mode(olm_base64::engine::DecodePaddingMode::Indifferent),
);
...
pub(crate) fn unpickle<T: for<'b> serde::Deserialize<'b>>(
    ciphertext: &str,
    pickle_key: &[u8; 32],
) -> Result<T, crate::olm::PickleError> {
    use zeroize::Zeroize;

    let cipher = crate::olm::cipher::Cipher::new_pickle(pickle_key);
...
pub(crate) fn extract_mac(slice: &[u8], truncated: bool) -> crate::olm::cipher::MessageMac {
    use crate::olm::cipher::Mac;
