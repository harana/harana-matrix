...
///
/// This type can be constructed from one of its constants (like
/// [`RoomVersionRules::V1`]), or from [`RoomVersionId::rules()`].
///
...
pub struct RoomVersionRules {
    ...
    /// ([spec]), introduced in room version 5.
    ///
...
pub enum EventIdFormatVersion {
    ...
    /// `$hash` format using standard unpadded base64 ([spec]), introduced in
    /// room version 3.
    ///
    /// [spec]: https://spec.matrix.org/v1.19/rooms/v3/#event-ids
    V2,

    /// `$hash` format using URL-safe unpadded base64 ([spec]), introduced in
    /// room version 4.
    ///
...
pub enum RoomIdFormatVersion {
    ...
    /// `!hash` format using the reference hash of the `m.room.create` event of
    /// the room, introduced in room version 12.
    V2,
}
...
pub enum EventsReferenceFormatVersion {
    /// `[["$id:server", {"sha256": "hash"}]]` format ([spec]), introduced in
    /// room version 1.
    ///
...
pub enum StateResolutionVersion {
    /// First version of the state resolution algorithm ([spec]), introduced in
    /// room version 1.
    ///
    /// [spec]: https://spec.matrix.org/v1.19/rooms/v1/#state-resolution
    V1,

    /// Second version of the state resolution algorithm ([spec]), introduced in
    /// room version 2.
    ///
...
impl StateResolutionVersion {
    /// Gets the `StateResolutionV2Rules` for the room version, if it uses the
    /// second version of the state resolution algorithm.
...
///
/// This type can be constructed from one of its constants (like
/// [`StateResolutionV2Rules::V2_0`]), or by constructing a [`RoomVersionRules`]
/// first and using the `state_res` field (if the room version uses version 2 of
...
pub struct StateResolutionV2Rules {
    /// Whether to begin the first phase of iterative auth checks with an empty
    /// state map, as opposed to one containing the unconflicted state,
    /// enabled since room version 12.
    pub begin_iterative_auth_checks_with_empty_state_map: bool,

    /// Whether to include the conflicted state subgraph in the full conflicted
    /// state, enabled since room version 12.
    pub consider_conflicted_state_subgraph: bool,
}

impl StateResolutionV2Rules {
    /// The first version of the second iteration of the state resolution
    /// algorithm, introduced in room version 2.
    pub const V2_0: Self = Self {
        begin_iterative_auth_checks_with_empty_state_map: false,
        consider_conflicted_state_subgraph: false,
    };

    /// The second version of the second iteration of the state resolution
    /// algorithm, introduced in room version 12.
...
///
/// This type can be constructed from one of its constants (like
/// [`AuthorizationRules::V1`]), or by constructing a [`RoomVersionRules`] first
/// and using the `authorization` field.
///
...
pub struct AuthorizationRules {
    ...
    /// events ([spec]), disabled since room version 3.
    ///
    /// [spec]: https://spec.matrix.org/v1.19/rooms/v3/#handling-redactions
    pub special_case_room_redaction: bool,

    /// Whether to apply special authorization rules for `m.room.aliases` events
    /// ([spec]), disabled since room version 6.
    ///
    /// [spec]: https://spec.matrix.org/v1.19/rooms/v6/#authorisation-rules
    pub special_case_room_aliases: bool,

    /// Whether to strictly enforce [canonical JSON] ([spec]), introduced in
    /// room version 6.
    ///
    /// Numbers in Canonical JSON must be integers in the range [-2<sup>53</sup>
    /// + 1, 2<sup>53</sup> - 1], represented without exponents or decimal
    ...
    /// Whether to check the `notifications` field when checking
    /// `m.room.power_levels` events ([spec]), introduced in room version 6.
    ///
    /// [spec]: https://spec.matrix.org/v1.19/rooms/v6/#authorisation-rules
    pub limit_notifications_power_levels: bool,

    /// Whether to allow the `knock` membership for `m.room.member` events and
    /// the `knock` join rule for `m.room.join_rules` events ([spec]),
    /// introduced in room version 7.
    ///
    /// [spec]: https://spec.matrix.org/v1.19/rooms/v7/#authorisation-rules
    pub knocking: bool,

    /// Whether to allow the `restricted` join rule for `m.room.join_rules`
    ...
    /// Whether to allow the `knock_restricted` join rule for
    /// `m.room.join_rules` events ([spec]), introduced in room version 10.
    ///
    ...
    /// events be integers ([spec]), introduced in room version 10.
    ///
    ...
    /// event's `sender`, instead of the event content's `creator` field
    /// ([spec]), introduced in room version 11.
    ///
    /// [spec]: https://spec.matrix.org/v1.19/rooms/v11/#event-format
    pub use_room_create_sender: bool,

    /// Whether room creators should always be considered to have "infinite"
    /// power level, introduced in room version 12.
    pub explicitly_privilege_room_creators: bool,

    /// Whether additional room creators can be set with the
    /// `content.additional_creators` field of an `m.room.create` event,
    /// introduced in room version 12.
    pub additional_room_creators: bool,

    /// Whether to use the event ID of the `m.room.create` event of the room as
    /// the room ID, introduced in room version 12.
...
///
/// This type can be constructed from one of its constants (like
/// [`RedactionRules::V1`]), or by constructing a [`RoomVersionRules`] first and
/// using the `redaction` field.
///
/// [redaction]: https://spec.matrix.org/v1.19/client-server-api/#redactions
#[derive(Debug, Clone)]
#[cfg_attr(not(ruma_unstable_exhaustive_types), non_exhaustive)]
pub struct RedactionRules {
    /// Whether to keep the `aliases` field in the `content` of `m.room.aliases`
    /// events ([spec]), disabled since room version 6.
    ///
    /// [spec]: https://spec.matrix.org/v1.19/rooms/v6/#redactions
    pub keep_room_aliases_aliases: bool,

    /// Whether to keep the `allow` field in the `content` of
    /// `m.room.join_rules` events ([spec]), introduced in room version 8.
    ///
    /// [spec]: https://spec.matrix.org/v1.19/rooms/v8/#redactions
    pub keep_room_join_rules_allow: bool,

    /// Whether to keep the `join_authorised_via_users_server` field in the
    /// `content` of `m.room.member` events ([spec]), introduced in room
    /// version 9.
    ///
    /// [spec]: https://spec.matrix.org/v1.19/rooms/v9/#redactions
    pub keep_room_member_join_authorised_via_users_server: bool,

    /// Whether to keep the `origin`, `membership` and `prev_state` fields a the
    /// top-level of all events ([spec]), disabled since room version 11.
    ///
    /// [spec]: https://spec.matrix.org/v1.19/rooms/v11/#redactions
    pub keep_origin_membership_prev_state: bool,

    /// Whether to keep the entire `content` of `m.room.create` events ([spec]),
    /// introduced in room version 11.
    ///
    /// [spec]: https://spec.matrix.org/v1.19/rooms/v11/#redactions
    pub keep_room_create_content: bool,

    /// Whether to keep the `redacts` field in the `content` of
    /// `m.room.redaction` events ([spec]), introduced in room version 11.
    ///
    /// [spec]: https://spec.matrix.org/v1.19/rooms/v11/#redactions
    pub keep_room_redaction_redacts: bool,

    /// Whether to keep the `invite` field in the `content` of
    /// `m.room.power_levels` events ([spec]), introduced in room version
    /// 11.
    ///
    /// [spec]: https://spec.matrix.org/v1.19/rooms/v11/#redactions
    pub keep_room_power_levels_invite: bool,

    /// Whether to keep the `signed` field in `third_party_invite` of the
    /// `content` of `m.room.member` events ([spec]), introduced in room
    /// version 11.
    ///
    /// [spec]: https://spec.matrix.org/v1.19/rooms/v11/#redactions
    ...
    /// Whether the `content.redacts` field should be used to determine the
    /// event an event redacts, as opposed to the top-level `redacts` field
    /// ([spec]), introduced in room version 11.
    ///
    /// [spec]: https://spec.matrix.org/v1.19/rooms/v11/#redactions
    pub content_field_redacts: bool,

    /// Whether to keep the `allow`, `deny` and `allow_ip_literals` in the
    /// `content` of `m.room.server_acl` events ([MSC2870]).
    ///
    /// [MSC2870]: https://github.com/matrix-org/matrix-spec-proposals/pull/2870
    #[cfg(feature = "unstable-msc2870")]
...
///
/// This type can be constructed from one of its constants (like
/// [`SignaturesRules::V1`]), or by constructing a [`RoomVersionRules`] first
/// and using the `signatures` field.
///
...
pub struct SignaturesRules {
    /// Whether to check the server of the event ID, disabled since room version
    /// 3.
    pub check_event_id_server: bool,

    /// Whether to check the server of the `join_authorised_via_users_server`
    /// field in the `content` of `m.room.member` events ([spec]),
...
///
/// This type can be constructed from one of its constants (like
/// [`EventFormatRules::V1`]), or by constructing a [`RoomVersionRules`] first
/// and using the `event_format` field.
#[derive(Debug, Clone)]
#[cfg_attr(not(ruma_unstable_exhaustive_types), non_exhaustive)]
pub struct EventFormatRules {
    ...
    /// Whether the `room_id` field is required on the `m.room.create` event,
    /// disabled since room version 12.
    pub require_room_create_room_id: bool,

    /// Whether the `m.room.create` event is allowed to be in the `auth_events`,
    /// disabled since room version 12.
...
pub struct RoomPowerLevelsRules {
    /// The creator(s) of the room which are considered to have "infinite" power
    /// level, introduced in room version 12.
    #[allow(clippy::disallowed_types)]
    pub privileged_creators: Option<HashSet<OwnedUserId>>,
}

impl RoomPowerLevelsRules {
    /// Creates a new `RoomPowerLevelsRules` from the given parameters, using
    /// the `creators` if the rule `explicitly_privilege_room_creators` is
    /// `true`.
    pub fn new(
