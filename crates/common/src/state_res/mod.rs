...
//! is valid and how it affects the room state. The purpose of this module is to
...
//!   implemented.
//!
//! Homeservers using this module **should not** advertise support for those
//! room versions.
//!
//! # Checks performed on receipt of a PDU
//!
//! This module used with [`crate::__ruma::signatures`] should allow to perform
//! all the [necessary checks on receipt of a PDU].
//!
...
//! 2. [`crate::__ruma::signatures::verify_event()`] - The event should be
//!    dropped on error. The PDU should be redacted before checking the
//!    authorization rules if the result is `Verified::Signatures`.
...
//! # Event helper types
//!
//! The types from [`crate::__ruma::events`] use strict deserialization rules
//! according to their definition in the specification, which means that they
//! also validate fields that are not checked when receiving a PDU. Since it is
//! not appropriate for servers to reject an event that passes those
...
//!
//! The types from [`crate::__ruma::events`] are still appropriate to be used to
//! create a new event, or to validate an event received from a client.
//!
