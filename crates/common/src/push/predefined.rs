...
impl ConditionalPushRule {
    ...
    /// Matches an event that's part of a thread, that is *not* subscribed to,
    /// by the current user.
    ///
    ...
    /// Matches an event that's part of a thread, that *is* subscribed to, by
    /// the current user.
    ///
    /// Thread subscriptions are defined in [MSC4306].
    ///
...
#[ruma_enum(rename_all(prefix = ".m.rule.", rule = "snake_case"))]
#[non_exhaustive]
