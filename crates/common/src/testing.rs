//! Helpers for tests, behind the `testing` feature.
...
macro_rules! init_tracing_for_tests {
    () => {
        ...
        #[$crate::testing::__macro_support::ctor]
        fn init_logging() {
            use tracing_subscriber::{layer::SubscriberExt, util::SubscriberInitExt};
            use $crate::testing::__macro_support::tracing_subscriber;
            ...
            tracing_subscriber::registry()
                .with(tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| {
                    ...
                    //
                    // Since tracing_subscriber does prefix matching, the `matrix=` directive
                    // takes effect for all the main crates (`base`, `crypto`
                    // and so on).
                    "info,matrix=debug".into()
                }))
