use crate::{
    __ruma::{
        AnyKeyName, CanonicalJsonObject, CanonicalJsonValue, OwnedSigningKeyId,
        SigningKeyAlgorithm,
...
///
/// ```rust
/// # use harana_matrix_common::{RoomVersionId, serde::base64::Base64};
/// # use harana_matrix_common::signatures::{hash_and_sign_event, Ed25519KeyPair};
/// #
/// const PKCS8: &str = "\
...
///
/// ```rust
/// use harana_matrix_common::CanonicalJsonObject;
/// use harana_matrix_common::signatures::sign_event;
...
/// # let key_pair = harana_matrix_common::signatures::Ed25519KeyPair::from_der(
/// #    der.as_bytes(),
/// #    "1".into(), // The "version" of the key.
/// # )?;
/// # let room_version_rules = || { harana_matrix_common::room_version_rules::RoomVersionRules::V6 };
///
/// // Deserialize an event from JSON.
...
///
/// ```rust
/// # use harana_matrix_common::serde::base64::Base64;
/// #
/// const PKCS8: &str = "\
///     MFECAQEwBQYDK2VwBCIEINjozvdfbsGEt6DD+7Uf4PiJ/YvTNXV2mIPc/\
///     tA0T+6tgSEA3TPraTczVkDPTRaX4K+AfUuyx7Mzq1UafTXypnl0t2k\
...
/// assert!(
///     harana_matrix_common::signatures::sign_json(
///         "domain", &key_pair, &mut value
///     )
///     .is_ok()
/// );
/// ```
...
where
    K: KeyPair,
{
    let json = to_canonical_json_string_for_signing(object)?;
    let signature = key_pair.sign(json.as_bytes());
...
mod tests {
    use super::Signature;
    use crate::__ruma::SigningKeyAlgorithm;
