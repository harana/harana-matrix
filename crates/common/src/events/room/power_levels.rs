...
use crate::{
    ...
    events::{
        EmptyStateKey, MessageLikeEventType, RedactContent, RedactedStateEventContent,
...
pub struct RoomPowerLevelsEventContent {
    ...
    #[serde(
        ...
        deserialize_with = "crate::__ruma::serde::deserialize_v1_powerlevel"
    )]
    pub ban: Int,
    ...
    #[serde(
        ...
        deserialize_with = "crate::__ruma::serde::btreemap_deserialize_v1_powerlevel_values"
    )]
    ...
    #[serde(
        default,
        skip_serializing_if = "crate::__ruma::serde::is_default",
        deserialize_with = "crate::__ruma::serde::deserialize_v1_powerlevel"
    )]
...
impl RoomPowerLevelsEventContent {
    /// Creates a new `RoomPowerLevelsEventContent` with all-default values for
    ...
    pub fn new(rules: &AuthorizationRules) -> Self {
        ...
        if rules.explicitly_privilege_room_creators {
            // Since v12, the default power level to send m.room.tombstone events is
            // increased to PL150.
...
///
/// Is either considered "infinite" if that user is a room creator, or an
/// integer if they are not.
#[derive(PartialEq, Copy, Clone, Eq, Debug)]
#[cfg_attr(not(ruma_unstable_exhaustive_types), non_exhaustive)]
pub enum UserPowerLevel {
    /// The user is considered to have "infinite" power level, due to being a
    /// room creator, from room version 12 onwards.
    Infinite,

    /// The user is either not a creator, or the room version is prior to 12,
...
///
/// This struct contains all the power levels settings from the specification
/// and can be constructed from several [`RoomPowerLevelsSource`]s, which means
/// that it can be used when wanting to inspect the power levels of a room,
/// regardless of whether the most recent power levels event is redacted or not,
/// or the room has no power levels event.
///
/// This can also be used to change the power levels of a room by mutating it
/// and then converting it to a [`RoomPowerLevelsEventContent`] using
/// `RoomPowerLevelsEventContent::try_from` / `.try_into()`. This allows to
/// validate the format of the power levels before sending them. Note
/// that the homeserver might still refuse the power levels changes depending on
/// the current power level of the sender.
#[derive(Clone, Debug)]
#[cfg_attr(not(ruma_unstable_exhaustive_types), non_exhaustive)]
pub struct RoomPowerLevels {
    ...
    /// This is a mapping from event type to power level required.
    ///
    /// When built from [`RoomPowerLevelsSource::None`], defaults to an empty
    /// map.
    pub events: BTreeMap<TimelineEventType, Int>,
    ...
    /// `explicitly_privilege_room_creators` field of [`AuthorizationRules`] is
    /// set to `true`. This would result in an error when trying to convert
    /// this to a [`RoomPowerLevelsEventContent`].
    ///
    /// When built from [`RoomPowerLevelsSource::None`]:
    ///
    /// * If `explicitly_privilege_room_creators` is set to `false` for the room
    ///   version, defaults to setting the power level to `100` for the
    ///   creator(s) of the room.
    ...
    /// This is a mapping from `key` to power level for that notifications key.
    ///
    /// When built from [`RoomPowerLevelsSource::None`], uses its `Default`
    /// implementation.
    pub notifications: NotificationPowerLevels,
...
impl RoomPowerLevels {
    /// Constructs `RoomPowerLevels` from `RoomPowerLevelsSource`,
    /// `AuthorizationRules` and the creators of a room.
    pub fn new(
...
/// An error encountered when trying to build a [`RoomPowerLevelsEventContent`]
/// from [`RoomPowerLevels`].
#[derive(Copy, Clone, Debug, PartialEq, Eq, thiserror::Error)]
#[non_exhaustive]
pub enum PowerLevelsError {
    /// A creator is in the `users` map, and it is not allowed by the current
    /// room version.
...
pub enum RoomPowerLevelsSource {
    /// Construct `RoomPowerLevels` from the non-redacted `m.room.power_levels`
    /// event content.
    Original(RoomPowerLevelsEventContent),
    /// Construct `RoomPowerLevels` from the redacted `m.room.power_levels`
    /// event content.
