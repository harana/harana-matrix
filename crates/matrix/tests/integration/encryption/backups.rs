...
async fn test_incremental_upload_of_keys_sliding_sync() -> TestResult {
    ...
    let sliding = client
        ...
        .add_list(
            matrix::SlidingSyncList::builder("all")
                .sync_mode(matrix::SlidingSyncMode::new_selective().add_range(0..=20)),
        )
        .build()
        .await?;
...
async fn test_a_backed_up_room_key_we_cannot_read_is_reported() -> TestResult {
    ...
    assert_matches!(
        error,
        matrix::Error::CorruptBackupRoomKey { session_id: reported } => {
...
-> TestResult {
    ...
    let outbound_group_session = OutboundGroupSession::new(
        ...
        room_id,
        base::crypto::EncryptionSettings::default(),
    )?;
