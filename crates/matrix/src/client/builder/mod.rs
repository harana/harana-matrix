...
impl ClientBuilder {
    ...
    /// use matrix::{Client, config::StoreConfig};
    ///
    ...
    /// [`sdk_common::cross_process_lock::CrossProcessLock`]). The
    ...
    #[instrument(skip_all, target = "matrix::client", fields(homeserver))]
    ...
    let store_config = match builder_config {
        ...
        BuilderStoreConfig::Sqlite { config, cache_path } => {
            ...
            let (state_store, event_cache_store, media_store, crypto_store) = try_join!(
                sqlite::SqliteStateStore::open_with_config(&config),
                sqlite::SqliteEventCacheStore::open_with_config(&config_with_cache_path),
                sqlite::SqliteMediaStore::open_with_config(&config_with_cache_path),
                sqlite::SqliteCryptoStore::open_with_config(&config),
            )?;
            #[cfg(not(feature = "e2e-encryption"))]
            let (state_store, event_cache_store, media_store) = try_join!(
                sqlite::SqliteStateStore::open_with_config(&config),
                sqlite::SqliteEventCacheStore::open_with_config(&config_with_cache_path),
                sqlite::SqliteMediaStore::open_with_config(&config),
            )?;
...
async fn build_indexeddb_store_config(
    ...
    let stores = indexeddb::IndexeddbStores::open(name, passphrase).await?;
...
pub enum ClientBuildError {
    ...
    #[cfg(feature = "indexeddb")]
    #[error(transparent)]
    IndexeddbStore(#[from] indexeddb::OpenStoreError),

    /// Error opening the sqlite store.
    #[cfg(feature = "sqlite")]
    #[error(transparent)]
    SqliteStore(#[from] sqlite::OpenStoreError),
...
pub(crate) mod tests {
    ...
    impl StoreProvider for TestStoreProvider {
        fn open_stores<'a>(
            &'a self,
            cross_process_lock_config: &'a CrossProcessLockConfig,
        ) -> sdk_common::BoxFuture<'a, Result<StoreConfig, StoreProviderError>> {
            Box::pin(async move {
