...
pub(crate) struct ClientInner {
    ...
    /// [`sdk_common::cross_process_lock::CrossProcessLock`]) when
...
impl Client {
    ...
    ///
    /// The SDK provides cross-process store locks (see
    /// [`sdk_common::cross_process_lock::CrossProcessLock`]) when this
    ...
    /// # let client = matrix::Client::builder()
    ...
    /// # let mut client = Client::new(homeserver).await?;
    /// use matrix::ruma::{api::client::profile, owned_user_id};
    ///
    ...
    ///
    /// ```no_run
    /// use matrix::ruma::api::FeatureFlag;
    /// # use matrix::{Client, config::SyncSettings};
    /// # use url::Url;
    /// # async {
    /// # let homeserver = Url::parse("http://localhost:8080")?;
    ...
    ///
    /// ```no_run
    /// # use matrix::{Client, ruma::api::client::authenticated_media::get_media_preview};
    ...
    /// # Examples
    /// ```no_run
    /// # use matrix::{Client, config::SyncSettings, ruma::api::client::rtc::RtcTransport};
    /// # use url::Url;
    /// # async {
    /// # let homeserver = Url::parse("http://localhost:8080")?;
    /// # let mut client = Client::new(homeserver).await?;
    ...
    /// # use matrix::{Client, config::SyncSettings, LoopCtrl};
    /// # use url::Url;
    /// # async {
    /// # let homeserver = Url::parse("http://localhost:8080").unwrap();
    /// # let mut client = Client::new(homeserver).await.unwrap();
    ...
    /// `cross_process_lock_config`.
    ///
    /// [`CrossProcessLock::new`]: sdk_common::cross_process_lock::CrossProcessLock::new
