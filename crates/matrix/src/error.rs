...
#[cfg(feature = "sqlite")]
impl From<sqlite::OpenStoreError> for Error {
    fn from(error: sqlite::OpenStoreError) -> Self {
        ClientBuildError::from(error).into()
    }
}

#[cfg(feature = "indexeddb")]
impl From<indexeddb::OpenStoreError> for Error {
    fn from(error: indexeddb::OpenStoreError) -> Self {
        ClientBuildError::from(error).into()
    }
}
...
mod tests {
    ...
    fn test_client_build_error_propagates_with_the_question_mark_operator() {
        // The point of the conversion: a function returning `matrix::Error` can
    ...
    fn test_open_store_error_propagates_with_the_question_mark_operator() {
        fn open() -> Result<(), sqlite::OpenStoreError> {
            Err(sqlite::OpenStoreError::MissingVersion)
        }
