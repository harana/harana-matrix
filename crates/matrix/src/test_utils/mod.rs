...
use base::{deserialized_responses::TimelineEvent, store::RoomLoadSettings};
use ruma::{
    api::MatrixVersion,
...
///
/// ```rust
/// use matrix::assert_recv_with_timeout;
/// use tokio::sync::mpsc;
///
/// # async {
...
/// ```
/// # async {
/// # use matrix::assert_next_eq_with_timeout;
...
/// # let room_id: ruma::OwnedRoomId = unreachable!();
/// # let event_id: ruma::OwnedEventId = unreachable!();
/// use matrix::assert_let_decrypted_state_event_content;
///
/// let room =
///     client.get_room(&room_id).expect("Bob should have received the invite");
///
