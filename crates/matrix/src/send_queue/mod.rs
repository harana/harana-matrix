...
//!   [`base::store::SentRequestKey`]) into the dependent request
...
use base::store::FinishGalleryItemInfo;
use base::{
        ...
        loop {
            ...
            let (queued_request, cancel_upload_rx) = match queue.peek_next_to_send().await {
                ...
                Err(err) => {
                    ...
                    sdk_common::sleep::sleep(STORE_ERROR_BACKOFF).await;
                    continue;
                }
            };
                                ...
                                let timeline_event = match Raw::from_json_string(
                                    ...
                                    Ok(event) => match encryption_info {
                                        ...
                                        Some(encryption_info) => {
                                            use base::deserialized_responses::DecryptedRoomEvent;
