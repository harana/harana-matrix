...
#[cfg(feature = "sqlite")]
use base::crypto::OlmMachineBuilder;
pub use base::crypto::{
...
pub enum BundleExportError {
    ...
    #[error(transparent)]
    OpenStoreError(#[from] sqlite::OpenStoreError),
    ...
    /// the store.
    #[error(transparent)]
    SecretExport(#[from] base::crypto::store::SecretsBundleExportError),
}
...
pub async fn export_secrets_bundle_from_store(
    ...
    use base::crypto::store::CryptoStore;

    let store = sqlite::SqliteCryptoStore::open(database_path, passphrase).await?;
    let account =
...
impl Client {
    ...
    pub(crate) async fn mark_request_as_sent(
        ...
        response: impl Into<base::crypto::types::requests::AnyIncomingResponse<'_>>,
    ) -> Result<(), base::Error> {
        Ok(self
            .olm_machine()
            .await
    ...
    ///     base::crypto::olm::CrossSigningStatus::is_published
    pub(crate) async fn mark_cross_signing_identity_as_published(
        &self,
        response: &UploadSigningKeysResponse,
    ) {
        use base::crypto::types::requests::AnyIncomingResponse;
    ...
    /// # use matrix::Client;
    /// # use url::Url;
    /// # use matrix::ruma::{room_id, OwnedRoomId};
    /// use serde::{Deserialize, Serialize};
    /// use matrix::ruma::events::{macros::EventContent, room::EncryptedFile};
    ///
    ...
    pub(crate) async fn send_verification_request(
        ...
        use base::crypto::types::requests::OutgoingVerificationRequest::*;
    ...
    async fn send_outgoing_request(&self, r: OutgoingRequest) -> Result<()> {
        use base::crypto::types::requests::AnyOutgoingRequest;
...
impl Encryption {
    ...
    pub async fn get_verification(&self, user_id: &UserId, flow_id: &str) -> Option<Verification> {
        ...
        olm.get_verification(user_id, flow_id).and_then(|v| match v {
            base::crypto::Verification::SasV1(sas) => {
                Some(SasVerification { inner: sas, client: self.client.clone() }.into())
            }
            #[cfg(feature = "qrcode")]
            base::crypto::Verification::QrV1(qr) => {
    ...
    ///
    /// ```no_run
    /// # use matrix::{Client, ruma::{device_id, user_id}};
    /// # use url::Url;
    /// # async {
    ...
    ///
    /// ```no_run
    /// # use matrix::{Client, ruma::user_id};
    /// # use url::Url;
    /// # async {
    /// # let alice = user_id!("@alice:example.org");
    /// # let homeserver = Url::parse("http://example.com")?;
    ...
    /// # use matrix::{ruma::api::client::uiaa, Client};
    /// # use url::Url;
    /// # use serde_json::json;
    /// # async {
    /// # let homeserver = Url::parse("http://example.com")?;
    ...
    ///
    /// ```no_run
    /// use matrix::{ruma::api::client::uiaa, encryption::CrossSigningResetAuthType};
    ///
    /// # async {
    /// # let homeserver = url::Url::parse("http://example.com")?;
    /// # let client = matrix::Client::new(homeserver).await?;
    ...
    ///             use matrix::ruma::api::client::uiaa;
    ///
    ...
    pub async fn export_room_keys(
        ...
        path: PathBuf,
        passphrase: &str,
        predicate: impl FnMut(&base::crypto::olm::InboundGroupSession) -> bool,
    ) -> Result<()> {
        let olm = self.client.olm_machine().await;
        let olm = olm.as_ref().ok_or(Error::NoOlmMachine)?;
        ...
        let encrypt = move || -> Result<()> {
            let export: String =
                base::crypto::encrypt_room_key_export(&keys, &passphrase, 500_000)?;
    ...
    pub async fn import_room_keys(
        ...
        let decrypt = move || {
            ...
            base::crypto::decrypt_room_key_export(file, &passphrase)
        };
...
mod tests {
    ...
    async fn test_a_rejected_key_upload_is_not_retried_forever() {
        ...
        let has_key_upload = async |machine: &base::crypto::OlmMachine| {
            machine
                .outgoing_requests()
                .await
                .unwrap()
    ...
    async fn test_generation_counter_invalidates_olm_machine() {
        ...
        use base::store::RoomLoadSettings;
        ...
        let decryption_key = base::crypto::store::types::BackupDecryptionKey::new();
    ...
    async fn test_a_rejected_duplicate_one_time_key_upload_is_not_retried_forever() {
        ...
        assert!(
            !outgoing.iter().any(|request| matches!(
                request.request(),
                base::crypto::types::requests::AnyOutgoingRequest::KeysUpload(_)
            )),
