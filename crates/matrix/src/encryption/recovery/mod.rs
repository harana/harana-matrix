...
//!
//! ```no_run
//! use matrix::{Client, encryption::EncryptionSettings};
//!
//! # async {
...
//!
//! ```no_run
//! # use matrix::{Client, encryption::recovery::EnableProgress};
//! # use url::Url;
//! # async {
...
impl Recovery {
    ...
    ///
    /// ```no_run
    /// # use matrix::{Client, encryption::recovery::RecoveryState};
    /// # use url::Url;
    /// # async {
    /// # let homeserver = Url::parse("http://example.com")?;
    /// # let client = Client::new(homeserver).await?;
