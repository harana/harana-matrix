...
impl PaginatedCache for ThreadEventCacheWrapper {
    ...
    async fn conclude_backwards_pagination_from_network(
        ...
        // ephemeral events not included in /relations responses, so there are no
        // receipt events to hand over here.
        //
        ...
        state.post_process_upserted_events(topo_ordered_events.iter(), receipt_events).await?;
