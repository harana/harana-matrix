...
pub use base::store::StoredThreadSubscription;
use base::{
...
impl Room {
    ...
    /// ```no_run
    /// # use matrix::Client;
    /// # use matrix::ruma::room_id;
    /// # use matrix::media::MediaFormat;
    /// # use url::Url;
    ...
    ///
    /// ```no_run
    /// use matrix::{Client, room::MessagesOptions};
    /// # use matrix::ruma::{
    ...
    /// # async {
    /// # let room: matrix::Room = todo!();
    /// # let user_ids: &[matrix::ruma::OwnedUserId] = &[];
    /// use matrix::ruma::events::room::member::RoomMemberEventContent;
    ///
    ...
    /// # async {
    /// # let room: matrix::Room = todo!();
    /// use matrix::ruma::events::room::power_levels::RoomPowerLevelsEventContent;
    ///
    ...
    /// # async {
    /// # let room: matrix::Room = todo!();
    /// use matrix::ruma::events::fully_read::FullyReadEventContent;
    ///
    ...
    /// # let mut client = matrix::Client::new(homeserver).await?;
    /// # let room_id = matrix::ruma::room_id!("!test:localhost");
    /// use matrix::ruma::events::tag::TagInfo;
    ///
    ...
    ///
    /// use matrix::ruma::api::client::typing::create_typing_event::v3::Typing;
    /// # use matrix::{
    ...
    /// # use matrix::{Client, config::SyncSettings};
    /// # use url::Url;
    /// # use matrix::ruma::room_id;
    ...
    /// ```no_run
    /// # use std::fs;
    /// # use matrix::{Client, ruma::room_id, attachment::AttachmentConfig};
    /// # use url::Url;
    /// # use mime;
    /// # async {
    /// # let homeserver = Url::parse("http://localhost:8080")?;
    ...
    /// # use serde::{Deserialize, Serialize};
    /// # async {
    /// # let joined_room: matrix::Room = todo!();
    /// use matrix::ruma::{
    ...
    /// result, use
    /// [`base::Room::cached_user_defined_notification_mode`].
    //
...
mod tests {
    ...
    async fn test_cache_invalidation_while_encrypt() {
        ...
        use sdk_test::{DEFAULT_TEST_ROOM_ID, message_like_event_content};
    ...
    async fn test_composer_draft() {
        use sdk_test::DEFAULT_TEST_ROOM_ID;
        ...
        let draft = ComposerDraft {
            ...
            attachments: vec![DraftAttachment {
                ...
                content: base::DraftAttachmentContent::File {
                    data: b"meow".to_vec(),
