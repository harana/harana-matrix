...
use client_common::{cross_process_lock::CrossProcessLockConfig, timer};
#[cfg(feature = "experimental-x509-identity-verification")]
use client_crypto::x509::{RawX509Signer, RawX509Verifier};
#[cfg(feature = "e2e-encryption")]
use client_crypto::{
...
/// rather through `client_matrix::Client`.
///
/// ```rust
/// use client_base::{
...
impl BaseClient {
    ...
    ///   [`client_crypto::vodozemac::olm::Account`] to be used for the
...
mod tests {
    ...
    async fn test_receive_sent_state_event_makes_it_readable_right_away() {
        ...
        let response = sync_builder
            .add_joined_room(common_test::JoinedRoomBuilder::new(room_id))
            .build_sync_response();
    ...
    async fn test_reinvited_members_get_a_display_name() {
        ...
        let response = sync_builder
            .add_joined_room(
                common_test::JoinedRoomBuilder::new(room_id)
    ...
    async fn test_room_member_carries_global_profile_status() {
        ...
        let response = sync_builder
            .add_joined_room(
                common_test::JoinedRoomBuilder::new(room_id).add_state_event(f.member(user_id)),
            )
            .build_sync_response();
        client.receive_sync_response(response).await.unwrap();
