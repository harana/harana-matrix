...
use client_base::crypto::store::types::Changes;
use client_common::cross_process_lock::CrossProcessLockConfig;
use common_test::async_test;
use client_ui::encryption_sync_service::{
...
async fn test_smoke_encryption_sync_works() -> anyhow::Result<()> {
    ...
    let encryption_sync = EncryptionSyncService::new(client, None, EncryptionSyncMode::App).await?;
...
async fn test_encryption_sync_always_reloads_todevice_token() -> anyhow::Result<()> {
    ...
    let sync_permit_guard = sync_permit.lock_owned().await;
    let encryption_sync =
        EncryptionSyncService::new(client.clone(), None, EncryptionSyncMode::App).await?;
    ...
    if let Some(olm_machine) = &*client.olm_machine_for_testing().await {
        olm_machine
            ...
            .save_changes(Changes {
                next_batch_token: Some(client_base::to_device_token::tag("nb2")),
                ..Default::default()
            })
            .await?;
    }
