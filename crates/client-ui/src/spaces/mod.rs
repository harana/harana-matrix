...
impl SpaceService {
    ...
    pub async fn add_child_to_space(
        ...
        let child_power_levels = child_room
            .power_levels()
            .await
            .map_err(|error| Error::UpdateRelationship(client_matrix::Error::from(error)))?;
    ...
    pub async fn remove_child_from_space(
        ...
        if let Some(child_room) = self.client.get_room(&child_id) {
            let power_levels = child_room.power_levels().await.map_err(|error| {
                Error::UpdateInverseRelationship(client_matrix::Error::from(error))
            })?;
...
mod tests {
    ...
    use client_matrix::{room::ParentSpace, test_utils::mocks::MatrixMockServer};
    use common_test::{
