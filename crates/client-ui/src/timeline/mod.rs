...
///
/// Like [`client_matrix::attachment::AttachmentConfig`], but instead of the
...
pub struct AttachmentConfig {
    ...
    /// it. See [`client_matrix::attachment::AttachmentConfig::strip_exif`].
    pub strip_exif: bool,
    /// Whether to compute the BlurHash of the image before uploading it. See
    /// [`client_matrix::attachment::AttachmentConfig::generate_blurhash`].
...
impl Timeline {
    ...
    /// # use client_matrix::{Client, config::SyncSettings, ruma::room_id};
    ...
    ///
    /// [`Media::get_media_content()`]: client_matrix::Media::get_media_content
    #[instrument(skip_all)]
...
///
/// This duplicates [`client_matrix::attachment::GalleryConfig`] but uses an
...
#[cfg(feature = "unstable-msc4274")]
impl TryFrom<GalleryItemInfo> for client_matrix::attachment::GalleryItemInfo {
    type Error = Error;

    fn try_from(value: GalleryItemInfo) -> Result<Self, Self::Error> {
        let (data, filename) = value.source.try_into_bytes_and_filename()?;
        Ok(client_matrix::attachment::GalleryItemInfo {
            filename,
