...
use client_base::{CallIntentConsensus, RoomInfo};
...
impl<P: RoomDataProvider> TimelineController<P> {
    ...
    /// [`client_base::RoomInfo`] and applies the new value to the
    ...
    pub(super) async fn set_sender_profiles_error(&self, error: Arc<client_matrix::Error>) {
                ...
                if let Some(send_error) = send_error {
                    self.update_event_send_state(
                        ...
                        EventSendState::SendingFailed {
                            error: Arc::new(client_matrix::Error::SendQueueWedgeError(Box::new(
                                send_error,
                            ))),
