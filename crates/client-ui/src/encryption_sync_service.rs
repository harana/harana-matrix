...
impl EncryptionSyncService {
    ...
    ///
    /// This will create and manage an instance of [`client_matrix::SlidingSync`].
    pub async fn new(
        client: Client,
    ...
    async fn new_inner(
            ...
            client.cross_process_lock_config()
        {
            ...
            match client.encryption().enable_cross_process_store_lock(holder_name.clone()).await {
                Ok(()) | Err(client_matrix::Error::BadCryptoStoreState) => {
...
pub enum Error {
    ...
    SlidingSync(client_matrix::Error),

    #[error("Locking failed: {0:#}")]
    LockError(client_matrix::Error),

    #[error(transparent)]
    ClientError(client_matrix::Error),
}
