...
async fn import_store_cipher_with_key(
    ...
    let cipher = match StoreCipher::import_with_key(chacha_key, serialised_cipher) {
        Ok(cipher) => cipher,
        Err(store_encryption::Error::KdfMismatch) => {
...
struct InboundGroupSessionIndexedDbObject {
    /// Possibly encrypted
    /// [`crypto::olm::group_sessions::PickledInboundGroupSession`]
...
mod durability_tests {
    use crypto::store::types::Changes;
    ...
    async fn test_only_session_writes_ask_for_strict_durability() {
        ...
        let mut alice = crypto::olm::Account::with_device_id(
            ruma::user_id!("@alice:localhost"),
            ruma::device_id!("ALICE"),
        );
        let mut bob = crypto::olm::Account::with_device_id(
...
#[cfg(all(test, target_family = "wasm"))]
mod tests {
    use crypto::cryptostore_integration_tests;
