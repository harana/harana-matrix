...
use crate::{
    base::{locks::Mutex, task_monitor::TaskMonitor},
    common::executor::{AbortOnDrop, JoinHandleExt as _, spawn},
};
...
impl BackPaginationHandle {
    ...
    ///
    /// Only the last handle for a request can disarm its cancellation; while
    /// other handles are alive they keep the request running anyway.
...
struct BackPaginationQueueInner {
    ...
    _task: crate::base::task_monitor::BackgroundTaskHandle,
}
