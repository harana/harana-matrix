...
#[cfg(feature = "experimental-encrypted-state-events")]
use crate::base::crypto::types::events::room::encrypted::EncryptedEvent;
#[cfg(feature = "e2e-encryption")]
...
pub use crate::base::store::StoredThreadSubscription;
#[cfg(feature = "e2e-encryption")]
use crate::base::{crypto::RoomEventDecryptionResult, deserialized_responses::EncryptionInfo};
#[cfg(feature = "e2e-encryption")]
...
impl Room {
    ...
    ///
    /// ```no_run
    /// use crate::{Client, room::MessagesOptions};
    /// # use crate::ruma::{
    ...
    /// # async {
    /// # let room: crate::Room = todo!();
    /// # let user_ids: &[crate::ruma::OwnedUserId] = &[];
    /// use crate::ruma::events::room::member::RoomMemberEventContent;
    ///
    ...
    /// # async {
    /// # let room: crate::Room = todo!();
    /// use crate::ruma::events::room::power_levels::RoomPowerLevelsEventContent;
    ///
    ...
    /// # async {
    /// # let room: crate::Room = todo!();
    /// use crate::ruma::events::fully_read::FullyReadEventContent;
    ///
    ...
    /// # let mut client = crate::Client::new(homeserver).await?;
    /// # let room_id = crate::ruma::room_id!("!test:localhost");
    /// use crate::ruma::events::tag::TagInfo;
    ///
    ...
    ///
    /// use crate::ruma::api::client::typing::create_typing_event::v3::Typing;
    /// # use crate::{
    ...
    /// # use crate::{Client, config::SyncSettings};
    /// # use url::Url;
    /// # use crate::ruma::room_id;
    ...
    /// ```no_run
    /// # use std::fs;
    /// # use crate::{Client, ruma::room_id, attachment::AttachmentConfig};
    /// # use url::Url;
    /// # use mime;
    /// # async {
    /// # let homeserver = Url::parse("http://localhost:8080")?;
    ...
    /// # use serde::{Deserialize, Serialize};
    /// # async {
    /// # let joined_room: crate::Room = todo!();
    /// use crate::ruma::{
    ...
    /// result, use
    /// [`crate::base::Room::cached_user_defined_notification_mode`].
    //
...
mod tests {
    ...
    use crate::{
        Client, Room,
        base::{ComposerDraft, DraftAttachment, store::ComposerDraftType},
        room::messages::{IncludeRelations, ListThreadsOptions, RelationsOptions},
        test::{JoinedRoomBuilder, SyncResponseBuilder, async_test, event_factory::EventFactory},
        test_utils::{
            logged_in_client,
    ...
    async fn test_cache_invalidation_while_encrypt() {
        use crate::{
            base::store::RoomLoadSettings,
            test::{DEFAULT_TEST_ROOM_ID, message_like_event_content},
        };
    ...
    async fn test_composer_draft() {
        use crate::test::DEFAULT_TEST_ROOM_ID;
        ...
        let draft = ComposerDraft {
            ...
            attachments: vec![DraftAttachment {
                ...
                content: crate::base::DraftAttachmentContent::File {
                    data: b"meow".to_vec(),
