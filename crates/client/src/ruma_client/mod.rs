...
//! `crate::ruma_client::http_client` for the generic parameter. For the client
//! API, there are login and registration methods provided for the client:
//!
//! ```no_run
//! # async {
//! # type HttpClient = crate::ruma_client::http_client::Dummy;
//! let homeserver_url = "https://example.com".to_owned();
//! let client =
//!     crate::ruma_client::Client::builder().homeserver_url(homeserver_url).build::<HttpClient>().await?;
//!
...
//! # Result::<(), crate::ruma_client::Error<_, _>>::Ok(())
//! # };
//! ```
//!
...
//! # type HttpClient = crate::ruma_client::http_client::Dummy;
//! let homeserver_url = "https://example.com".to_owned();
//! let client = crate::ruma_client::Client::builder()
...
//! it any of the `Request` types found in `harana_matrix_common::api::*` and
//! get back a corresponding response from the homeserver.
//!
//! For example:
//!
//! ```no_run
