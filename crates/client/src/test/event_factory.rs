...
use crate::{
    common::deserialized_responses::{TimelineEvent, UnableToDecryptInfo, UnableToDecryptReason},
...
impl EventFactory {
    ...
    pub fn space_order(&self, order: &str) -> EventBuilder<SpaceOrderEventContent> {
        let order =
            harana_matrix_common::SpaceChildOrder::parse(order).expect("order should be valid");
