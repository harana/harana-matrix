...
use crate::{
    Client, Error, Room,
    base::{
        RawStateEventWithKeys, RoomHeroWithProfile, RoomInfo, RoomInfoNotableUpdateReasons,
        RoomState,
    },
...
impl RoomPreview {
    ...
    pub async fn from_state_events(client: &Client, room_id: &RoomId) -> crate::Result<Self> {
        ...
        for ev in state.room_state {
            if let Some(mut raw_event) = RawStateEventWithKeys::try_from_raw_state_event(ev.cast())
            {
...
mod tests {
    use harana_matrix_common::{
        RoomOrAliasId, ServerName, owned_server_name, room_alias_id, room_id, server_name,
    };
