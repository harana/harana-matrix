...
use crate::{
    base::deserialized_responses::TimelineEvent,
    test::{
...
/// use crate::{ruma::{room_id, event_id}, test_utils::mocks::MatrixMockServer};
/// use serde_json::json;
///
...
impl MatrixMockServer {
    ...
    /// # tokio_test::block_on(async {
    /// use crate::{ruma::{room_id, event_id}, test_utils::mocks::MatrixMockServer};
    /// use crate::test::LeftRoomBuilder;
    ///
    /// let mock_server = MatrixMockServer::new().await;
    /// let client = mock_server.client_builder().build().await;
    ///
    ...
    /// ```
    /// # tokio_test::block_on(async {
    /// use crate::{ruma::room_id, test_utils::mocks::MatrixMockServer};
    ///
    /// let mock_server = MatrixMockServer::new().await;
    /// let client = mock_server.client_builder().build().await;
    ///
...
impl MatrixMockServer {
    ...
    /// # tokio_test::block_on(async {
    /// use crate::{
    ///     ruma::room_id, test::JoinedRoomBuilder,
    ...
    /// ```
    /// # tokio_test::block_on(async {
    /// use crate::{ruma::room_alias_id, test_utils::mocks::MatrixMockServer};
    ///
    /// let mock_server = MatrixMockServer::new().await;
    /// let client = mock_server.client_builder().build().await;
    ///
    ...
    /// use crate::room_directory_search::RoomDirectorySearch;
    /// use crate::{
    ///     ruma::{event_id, room_id},
    ///     test_utils::mocks::MatrixMockServer,
    /// };
    ...
    ///
    /// use crate::{
    ///     base::RoomMemberships,
    ///     ruma::{event_id, room_id},
    ///     test::event_factory::EventFactory,
    ///     test_utils::mocks::MatrixMockServer,
    /// };
    /// let mock_server = MatrixMockServer::new().await;
    /// let client = mock_server.client_builder().build().await;
