...
use harana_matrix_common::{
    OwnedDeviceId, OwnedUserId, api::MatrixVersion, owned_device_id, owned_user_id,
};

use crate::{
    Client, ClientBuilder, SessionTokens,
    authentication::matrix::MatrixSession,
    base::{SessionMeta, store::RoomLoadSettings},
...
impl MockClientBuilder {
    ...
    ///             .cross_process_store_config(CrossProcessLockConfig::MultiProcess { holder_name: "hodor".to_owned()})
    ///     })
    ///     .build()
    ///     .await;
    /// # anyhow::Ok(()) });
