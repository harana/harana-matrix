...
```rust,no_run
# use client_matrix::{Client, sliding_sync::Version};
# use url::Url;
# async {
...
```rust
# use client_matrix::sliding_sync::{SlidingSyncList, SlidingSyncMode};
...
the [`BaseClient`][`client_base::BaseClient`] as in previous sync. This
...
`timeline` API from `ui`. This is prefilled with already received
data.
...
```rust,no_run
use client_matrix::{Client, sliding_sync::{SlidingSyncList, SlidingSyncMode, Version}};
