...
use crate::common::executor::JoinHandleExt as _;
use crate::{
    Client, Result,
    base::RequestedRequiredStates,
    common::{executor::spawn, timer},
...
mod tests {
    ...
    use crate::{
        Client, Result,
        base::{RequestedRequiredStates, RoomMemberships},
        common::executor::spawn,
        test::{ALICE, async_test, event_factory::EventFactory},
    ...
    async fn test_extensions_to_device_since_is_set() {
        use crate::base::crypto::store::types::Changes;
...

        {
            if let Some(olm_machine) = &*client.olm_machine().await {
                olm_machine
                    .store()
                    .save_changes(Changes {
                        next_batch_token: Some(crate::base::to_device_token::tag(&since_token)),
                        ..Default::default()
                    })
                    .await
                    .unwrap();
    ...
    async fn test_no_pos_without_marking_tracked_users_dirty() -> anyhow::Result<()> {
        ...
        use crate::{
            base::crypto::types::requests::{AnyIncomingResponse, AnyOutgoingRequest},
...

        {
            ...
            assert_eq!(
                olm_machine
                    ...
                    .and_then(crate::base::to_device_token::untag),
