...
//! is re-exported here as [`crate::ruma`][ruma] so that using them doesn't
...
//! [`Raw`][types::Raw], are collected in [`crate::types`][types], which is
//! the place to start.
//!
//! # Matrix versions
//!
...
//! [`MatrixVersion`][harana_matrix_common::api::MatrixVersion]. That is the
//! ceiling: it says what the SDK knows how to speak, not what any given
//! homeserver answers to.
//!
...
pub use crate::{
    base::{
        CallIntentConsensus, ComposerDraft, ComposerDraftType, DraftAttachment,
        DraftAttachmentContent, DraftThumbnail, EncryptionState, PredecessorRoom, QueueWedgeError,
        Room as BaseRoom, RoomCreateWithCreatorEventContent, RoomDisplayName, RoomHero,
        RoomHeroWithProfile, RoomInfo, RoomMember as BaseRoomMember, RoomMembersUpdate,
        RoomMemberships, RoomRecencyStamp, RoomState, SessionMeta, StateChanges, StateStore,
        StoreError, SuccessorRoom, ThreadingSupport, deserialized_responses,
...
pub use crate::sqlite::SqliteCryptoStore;
#[cfg(feature = "sqlite")]
pub use crate::sqlite::log_targets as sqlite_log_targets;
#[cfg(feature = "sqlite")]
...
// The crates this one was merged from, one module each.
pub mod base;
pub mod common;
...
#[cfg(feature = "contentscanner")]
pub mod contentscanner;
#[cfg(feature = "ruma-client")]
pub mod ruma_client;
#[cfg(feature = "experimental-search-core")]
pub mod search;
#[cfg(feature = "ui")]
