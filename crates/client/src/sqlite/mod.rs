...
use crate::store_encryption::{StoreCipherProvider, StoreCodec};

/// The cipher and codec traits [`SqliteStoreConfig`] takes, re-exported so
/// that using them does not mean depending on
/// [`crate::store_encryption`] directly.
pub mod pluggable {
    pub use crate::store_encryption::{
...
pub mod log_targets {
    ...
    ///
    /// [`SqliteEventCacheStore`]: crate::sqlite::SqliteEventCacheStore
    pub const EVENT_CACHE_STORE: &str = "crate::sqlite::event_cache_store";

    /// The target [`SqliteStateStore`] logs under.
    ///
    /// [`SqliteStateStore`]: crate::sqlite::SqliteStateStore
    pub const STATE_STORE: &str = "crate::sqlite::state_store";

    /// The target [`SqliteCryptoStore`] logs under.
    ///
    /// [`SqliteCryptoStore`]: crate::sqlite::SqliteCryptoStore
    pub const CRYPTO_STORE: &str = "crate::sqlite::crypto_store";

    /// The target [`SqliteMediaStore`] logs under.
    ///
    /// [`SqliteMediaStore`]: crate::sqlite::SqliteMediaStore
    pub const MEDIA_STORE: &str = "crate::sqlite::media_store";
}
