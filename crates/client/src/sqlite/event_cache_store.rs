...
async fn run_migrations(conn: &SqliteAsyncConn, version: u8) -> Result<()> {
    ...
    if version < 2 {
        ...
        conn.with_transaction(|txn| {
            txn.execute_batch(include_str!(
                "../../migrations/event_cache_store/002_lease_locks.sql"
            ))?;
    ...
    if version < 10 {
        ...
        conn.with_transaction(|txn| {
            txn.execute_batch(include_str!(
                "../../migrations/event_cache_store/010_drop_media.sql"
            ))?;
    ...
    if version < 18 {
        ...
        conn.with_transaction(|txn| {
            txn.execute_batch(include_str!(
                "../../migrations/event_cache_store/018_key_value.sql"
            ))?;
...
mod tests {
    ...
    use crate::{
        ...
        test::{DEFAULT_TEST_ROOM_ID, async_test},
    };
    ...
    async fn test_linked_chunk_update_is_a_transaction() {
        ...
        assert_matches!(err, crate::sqlite::error::Error::Sqlite(err) => {
...
mod close_reopen_tests {
    ...
    use super::SqliteEventCacheStore;
    use crate::{
        base::{event_cache::store::EventCacheStore, linked_chunk::LinkedChunkId},
        test::{DEFAULT_TEST_ROOM_ID, async_test},
    };
