...
use crate::{
    common::{executor::spawn_blocking, sleep::sleep},
    sqlite::sync_wrapper::SyncWrapper,
};
...
/// store will fail with [`crate::sqlite::error::Error::StoreClosed`] until
...
pub(crate) async fn reopen_connections(
    ...
    runtime_config: crate::sqlite::RuntimeConfig,
) -> crate::sqlite::error::Result<()> {
    use crate::sqlite::utils::SqliteAsyncConnExt as _;
    ...
    let pool = Pool::builder(Manager::new(db_path)).config(pool_config).build().map_err(|e| {
        crate::sqlite::error::Error::InvalidData {
