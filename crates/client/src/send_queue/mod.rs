...
//!   [`crate::base::event_cache::store::EventCacheStore`], using an MXC ID that
//!   is temporary and designates a local URI without any possible doubt.
...
//!   [`crate::base::store::SentRequestKey`]) into the dependent request
...
use crate::base::crypto::{OlmError, SessionRecipientCollectionError};
pub use crate::base::store::EnforceThreadInReply;
#[cfg(feature = "unstable-msc4274")]
use crate::base::store::FinishGalleryItemInfo;
use crate::{
    ...
    client::WeakClient,
    common::{boxed_into_future, locks::Mutex as SyncMutex},
...
impl RoomSendQueue {
    ...
    pub async fn set_unread_marker(&self, unread: bool) -> Result<(), RoomSendQueueError> {
        ...
        match Raw::<harana_matrix_common::events::AnyRoomAccountDataEvent>::from_json_string(
            json.to_string(),
        ) {
            Ok(event) => {
        ...
        loop {
            ...
            let (queued_request, cancel_upload_rx) = match queue.peek_next_to_send().await {
                ...
                Err(err) => {
                    ...
                    crate::common::sleep::sleep(STORE_ERROR_BACKOFF).await;
                    continue;
                }
            };
                                ...
                                let timeline_event = match Raw::from_json_string(
                                    ...
                                    Ok(event) => match encryption_info {
                                        ...
                                        Some(encryption_info) => {
                                            use crate::base::deserialized_responses::DecryptedRoomEvent;
...
mod tests {
    ...
    use crate::{
        ...
        client::WeakClient,
        test::{JoinedRoomBuilder, SyncResponseBuilder, async_test},
    ...
    async fn test_which_wedged_requests_verification_can_unblock() {
        ...
        use crate::base::store::QueueWedgeError;
