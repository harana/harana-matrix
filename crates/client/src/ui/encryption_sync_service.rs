...
use crate::{
    Client, LEASE_DURATION_MS, SlidingSync, common::cross_process_lock::CrossProcessLockConfig,
    sleep::sleep,
};
...
impl EncryptionSyncService {
    ...
    ///
    /// This will create and manage an instance of [`crate::SlidingSync`].
    pub async fn new(
        client: Client,
    ...
    async fn new_inner(
            ...
            client.cross_process_lock_config()
        {
            ...
            match client.encryption().enable_cross_process_store_lock(holder_name.clone()).await {
                Ok(()) | Err(crate::Error::BadCryptoStoreState) => {
...
pub enum Error {
    ...
    SlidingSync(crate::Error),

    #[error("Locking failed: {0:#}")]
    LockError(crate::Error),
