...
pub use crate::ui::spaces::{room::SpaceRoom, room_list::SpaceRoomList};
use crate::{
    Client, Error as SDKError, Room,
    deserialized_responses::SyncOrStrippedState,
    task_monitor::BackgroundTaskHandle,
    ui::spaces::{graph::SpaceGraph, leave::LeaveSpaceHandle, room::SpaceRoomChildState},
};
...
/// use crate::Client;
/// use crate::ui::spaces::SpaceService;
...
impl SpaceService {
    ...
    /// can be used in tandem with the [`crate::ui::RoomListService`] to narrow
    /// down the presented rooms. A [`crate::ui::room_list_service::RoomList`]'s
    /// [`crate::ui::room_list_service::RoomListDynamicEntriesController`] can
    /// take a filter, which in this case can be a
    /// [`crate::ui::room_list_service::filters::new_filter_identifiers`]
    ...
    pub async fn add_child_to_space(
        ...
        let child_power_levels = child_room
            .power_levels()
            .await
            .map_err(|error| Error::UpdateRelationship(crate::Error::from(error)))?;
    ...
    pub async fn remove_child_from_space(
        ...
        if let Some(child_room) = self.client.get_room(&child_id) {
            let power_levels = child_room
                .power_levels()
                .await
                .map_err(|error| Error::UpdateInverseRelationship(crate::Error::from(error)))?;
...
mod tests {
    ...
    use crate::{
        room::ParentSpace,
        test::{JoinedRoomBuilder, LeftRoomBuilder, async_test, event_factory::EventFactory},
