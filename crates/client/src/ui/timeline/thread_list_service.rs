...
use crate::{
    Result, Room,
    common::serde_helpers::extract_thread_root,
    ...
    ui::timeline::{Profile, TimelineDetails, TimelineItemContent, traits::RoomDataProvider},
};
...
/// use crate::{
///     Room,
///     ui::timeline::thread_list_service::{
...
impl ThreadListService {
            ...
            .spawn_infinite_task("thread_list_service::event_cache_listener", {
                ...
                async move {
                    ...
                    let (_event_cache_drop, mut subscriber) = match async {
                        ...
                        crate::event_cache::Result::Ok((drop_handles, subscriber))
                    }
                    .await
                    {
                        Ok(pair) => pair,
    ...
    fn collect_events_from_diffs(
        diffs: Vec<VectorDiff<crate::base::event_cache::Event>>,
    ) -> Vec<crate::base::event_cache::Event> {
...
mod tests {
    ...
    use crate::{
        ...
        ui::timeline::{MsgLikeContent, MsgLikeKind, TimelineItemContent},
    };
