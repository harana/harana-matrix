...
use crate::{
    common::deserialized_responses::TimelineEvent,
    deserialized_responses::{
        AlgorithmInfo, DecryptedRoomEvent, EncryptionInfo, VerificationState,
    },
    test::{ALICE, DEFAULT_TEST_ROOM_ID, async_test, event_factory::EventFactory},
    ui::timeline::{
...
async fn test_live_location_no_sent_in_clear_shield() {
    ...
    let ts_enc =
        harana_matrix_common::MilliSecondsSinceUnixEpoch(harana_matrix_common::uint!(500_000));
    ...
    let ts =
        harana_matrix_common::MilliSecondsSinceUnixEpoch(harana_matrix_common::uint!(1_000_000));
    let beacon_event =
