...
use crate::{
    Client,
    base::{RoomInfo, RoomState, crypto::types::events::CryptoContextInfo},
    ...
    ui::unable_to_decrypt_hook::UtdHookManager,
};
...
impl PaginableThread for TestRoomDataProvider {
    async fn relations(
        ...
        _opts: crate::room::RelationsOptions,
    ) -> Result<Relations, crate::Error> {
        unimplemented!();
    }

    async fn load_event(&self, _event_id: &OwnedEventId) -> Result<TimelineEvent, crate::Error> {
        unimplemented!();
    }
}
...
impl RoomDataProvider for TestRoomDataProvider {
    ...
    async fn load_event<'a>(&'a self, event_id: &'a EventId) -> crate::Result<TimelineEvent> {
        self.loaded_events.write().await.push(event_id.to_owned());
        self.loadable_events
            .get(event_id)
            .cloned()
            .ok_or_else(|| crate::Error::UnknownError("event not found".into()))
    }
    ...
    ) -> crate::Result<(TimelineEvent, Vec<TimelineEvent>)> {
        unimplemented!();
    }
}
