...
use harana_matrix_common::{
    EventId, OwnedEventId, OwnedRoomId, events::BooleanType, push::Action, serde::Raw,
};
...
///
/// It is not meant to be implemented outside of matrix.
...
///
/// One line per event, at `TRACE` on the `crate::event_handler` target, so
...
mod tests {
    ...
    use crate::{
        Client, Room,
        common::{deserialized_responses::EncryptionInfo, locks::Mutex},
        event_handler::Ctx,
        test::SyncResponseBuilder,
    ...
    async fn test_each_received_event_is_logged() {
            ...
            S: Subscriber + for<'a> LookupSpan<'a>,
        {
            fn on_event(&self, event: &tracing::Event<'_>, _ctx: Context<'_, S>) {
                ...
                if *metadata.level() != Level::TRACE || metadata.target() != "crate::event_handler"
                {
                    return;
                }
