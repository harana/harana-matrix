...
//!
//! These come from [`harana_matrix_common`], which models the Matrix
//! specification. They are re-exported here so that using the SDK doesn't
//! require adding a direct dependency on `ruma` and matching its version:
//! `crate::ruma` (or [`crate::common::ruma`]) gives access to all of it, and
//! this module collects the handful of types that turn up in nearly every SDK
//! signature.
//!
//! # Identifiers
//!
...
//!
//! ```
//! use crate::common::{ruma::user_id, types::UserId};
//!
...
mod tests {
    use super::*;
    use crate::common::ruma::{events::AnySyncTimelineEvent as RumaAnySyncTimelineEvent, user_id};
