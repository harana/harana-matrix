...
//! of the SDK goes through [`crate::common::executor`],
//! [`crate::common::sleep`] and [`crate::common::timeout`], which are built on
//! top of it.
//!
...
use crate::common::{BoxFuture, SendOutsideWasm, SyncOutsideWasm};
...
pub trait AsyncRuntime: fmt::Debug + SendOutsideWasm + SyncOutsideWasm + 'static {
    ...
    /// of this by [`crate::common::executor::spawn`], so implementations don't
...
fn default_runtime() -> Arc<dyn AsyncRuntime> {
    ...
    panic!(
        "no async runtime is available: the `tokio-runtime` feature of \
         `sdk-common` is disabled, so `crate::common::runtime::set_runtime()` \
