...
#[cfg(feature = "experimental-element-recent-emojis")]
use crate::base::recent_emojis::RecentEmojisContent;
use crate::{
    Client, Error, Result,
    base::{
...
impl Account {
    ...
    /// ```no_run
    /// # use crate::Client;
    /// # use crate::ruma::{ClientSecret, uint};
    /// # use url::Url;
    /// # async {
    /// # let homeserver = Url::parse("http://localhost:8080")?;
    /// # let client = Client::new(homeserver).await?;
    ...
    /// ```no_run
    /// # use crate::Client;
    /// # use crate::ruma::thirdparty::Medium;
    /// # use crate::ruma::api::client::account::ThirdPartyIdRemovalStatus;
    /// # use url::Url;
    /// # async {
    /// # let homeserver = Url::parse("http://localhost:8080")?;
    /// # let client = Client::new(homeserver).await?;
    ...
    /// # let account = client.account();
    /// use crate::ruma::events::ignored_user_list::IgnoredUserListEventContent;
    ///
    /// let maybe_content =
    ...
    where
        C: GlobalAccountDataEventContent
            + StaticEventContent<IsPrefix = harana_matrix_common::events::False>,
    {
    ...
    /// # let client = Client::new("http://localhost:8080".parse()?).await?;
    /// # let account = client.account();
    /// use crate::ruma::events::{ignored_user_list::IgnoredUserListEventContent, GlobalAccountDataEventType};
    ///
    ...
    /// # use crate::Client;
    /// # use crate::ruma::events::media_preview_config::MediaPreviews;
    /// # use url::Url;
    /// # async {
    /// # let homeserver = Url::parse("http://localhost:8080")?;
    /// # let client = Client::new(homeserver).await?;
...
mod tests {
    ...
    use crate::{Error, test::async_test, test_utils::client::MockClientBuilder};
...
mod test_recent_emojis {
    ...
    use crate::{
        account::MAX_RECENT_EMOJI_COUNT,
        base::recent_emojis::RecentEmojisContent,
        config::SyncSettings,
