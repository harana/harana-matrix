...
use harana_matrix_common::{
    ...
    olm::Curve25519PublicKey,
};
#[cfg(feature = "experimental-send-custom-to-device")]
use harana_matrix_common::{
    events::AnyToDeviceEventContent, serde::Raw, to_device::DeviceIdOrAllDevices,
};
...
#[cfg(feature = "experimental-send-custom-to-device")]
use crate::base::crypto::CollectStrategy;
use crate::{
    ...
    base::{
        ...
        crypto::{
            ...
            types::{
                ...
                requests::{
                    OutgoingRequest, OutgoingVerificationRequest, RoomMessageRequest,
                    ToDeviceRequest,
                },
            },
        },
...
#[cfg(feature = "sqlite")]
use crate::base::crypto::OlmMachineBuilder;
use crate::common::cross_process_lock::CrossProcessLockConfig;
#[cfg(feature = "experimental-send-custom-to-device")]
use crate::config::RequestConfig;
pub use crate::{
    base::crypto::{
        CrossSigningStatus, CryptoStoreError, DecryptorError, EventError, KeyExportError,
        LocalTrust, MediaEncryptionInfo, MegolmError, OlmError, RoomKeyImportResult,
...
pub enum BundleExportError {
    ...
    #[error(transparent)]
    OpenStoreError(#[from] crate::sqlite::OpenStoreError),
    ...
    /// the store.
    #[error(transparent)]
    SecretExport(#[from] crate::base::crypto::store::SecretsBundleExportError),
}
...
/// `matrix`-based client in order to import the [`SecretsBundle`] in
...
pub async fn export_secrets_bundle_from_store(
    ...
    use crate::base::crypto::store::CryptoStore;

    let store = crate::sqlite::SqliteCryptoStore::open(database_path, passphrase).await?;
    let account =
...
impl Client {
    ...
    pub(crate) async fn mark_request_as_sent(
        ...
        response: impl Into<crate::base::crypto::types::requests::AnyIncomingResponse<'_>>,
    ...
    ///     crate::base::crypto::olm::CrossSigningStatus::is_published
    pub(crate) async fn mark_cross_signing_identity_as_published(
        &self,
        response: &UploadSigningKeysResponse,
    ) {
        use crate::base::crypto::types::requests::AnyIncomingResponse;
    ...
    /// Construct a
    /// [`EncryptedFile`][harana_matrix_common::events::room::EncryptedFile] by
    ...
    /// # use crate::Client;
    /// # use url::Url;
    /// # use crate::ruma::{room_id, OwnedRoomId};
    /// use serde::{Deserialize, Serialize};
    /// use crate::ruma::events::{macros::EventContent, room::EncryptedFile};
    ///
    ...
    pub(crate) async fn send_verification_request(
        ...
        use crate::base::crypto::types::requests::OutgoingVerificationRequest::*;
    ...
    async fn send_outgoing_request(&self, r: OutgoingRequest) -> Result<()> {
        use crate::base::crypto::types::requests::AnyOutgoingRequest;
...
impl Encryption {
    ...
    pub async fn get_verification(&self, user_id: &UserId, flow_id: &str) -> Option<Verification> {
        ...
        olm.get_verification(user_id, flow_id).and_then(|v| match v {
            crate::base::crypto::Verification::SasV1(sas) => {
                Some(SasVerification { inner: sas, client: self.client.clone() }.into())
            }
            #[cfg(feature = "qrcode")]
            crate::base::crypto::Verification::QrV1(qr) => {
    ...
    ///
    /// ```no_run
    /// # use crate::{Client, ruma::{device_id, user_id}};
    /// # use url::Url;
    /// # async {
    ...
    /// # use crate::{ruma::api::client::uiaa, Client};
    /// # use url::Url;
    /// # use serde_json::json;
    /// # async {
    /// # let homeserver = Url::parse("http://example.com")?;
    ...
    ///
    /// ```no_run
    /// use crate::{ruma::api::client::uiaa, encryption::CrossSigningResetAuthType};
    ///
    /// # async {
    /// # let homeserver = url::Url::parse("http://example.com")?;
    /// # let client = crate::Client::new(homeserver).await?;
    ...
    ///             use crate::ruma::api::client::uiaa;
    ///
    ...
    pub async fn export_room_keys(
        ...
        path: PathBuf,
        passphrase: &str,
        predicate: impl FnMut(&crate::base::crypto::olm::InboundGroupSession) -> bool,
    ) -> Result<()> {
        let olm = self.client.olm_machine().await;
        let olm = olm.as_ref().ok_or(Error::NoOlmMachine)?;
        ...
        let encrypt = move || -> Result<()> {
            let export: String =
                crate::base::crypto::encrypt_room_key_export(&keys, &passphrase, 500_000)?;
    ...
    pub async fn import_room_keys(
        ...
        let decrypt = move || {
            ...
            crate::base::crypto::decrypt_room_key_export(file, &passphrase)
        };
...
mod tests {
    ...
    async fn test_a_rejected_key_upload_is_not_retried_forever() {
        ...
        let has_key_upload = async |machine: &crate::base::crypto::OlmMachine| {
            machine
                .outgoing_requests()
                .await
                .unwrap()
    ...
    async fn test_generation_counter_invalidates_olm_machine() {
        ...
        use crate::base::store::RoomLoadSettings;
        ...
        let decryption_key = crate::base::crypto::store::types::BackupDecryptionKey::new();
    ...
    async fn test_a_rejected_duplicate_one_time_key_upload_is_not_retried_forever() {
        ...
        assert!(
            !outgoing.iter().any(|request| matches!(
                request.request(),
                crate::base::crypto::types::requests::AnyOutgoingRequest::KeysUpload(_)
            )),
