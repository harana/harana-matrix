...
use crate::{
    Client, Error, Room, base::crypto::olm::ExportedRoomKey, encryption::BackupDownloadStrategy,
};
...
impl Backups {
    ...
    ///
    /// ```no_run
    /// # use crate::{Client, encryption::backups::BackupState};
    /// # use url::Url;
    /// # async {
    ...
    ///
    /// ```no_run
    /// # use crate::{Client, encryption::backups::UploadState};
    /// # use url::Url;
    /// # async {
    /// # let homeserver = Url::parse("http://example.com")?;
    /// # let client = Client::new(homeserver).await?;
    ...
    async fn delete_backup_from_server(&self, version: String) -> Result<(), Error> {
        let request =
            harana_matrix_common::api::client::backup::delete_backup_version::v3::Request::new(
                version,
            );
...
mod test {
    ...
    use harana_matrix_common::{olm::Curve25519PublicKey, room_id};
    use serde_json::json;
    use wiremock::{
    ...
    async fn test_resuming_backups_when_keys_are_inconsistent_has_no_effect() {
        ...
        // crate::encryption::backups::Backups::maybe_enable_backups, and
    ...
    async fn test_the_done_of_an_earlier_upload_does_not_end_the_wait() {
        ...
        let result = crate::common::timeout::timeout(
    ...
    async fn test_waiting_for_steady_state_resets_the_delay() {
        ...
        let task = crate::common::executor::spawn({
