...
use crate::{
    Client, HttpError, HttpResult,
    base::{StateStoreDataKey, StateStoreDataValue, StoreError, ttl::TtlValue},
...
impl HomeserverCapabilities {
    ...
    async fn homeserver_capabilities_cached(&self) -> Result<Option<Capabilities>, StoreError> {
        ...
        if value.has_expired_after(self.client.discovery_cache_timeout()) {
        ...
        let mut capabilities_guard = match capabilities_cache.refresh_lock.try_lock() {
            ...
            Err(_) => {
                ...
                if let CachedValue::Cached(value) = capabilities_cache.value()
                    && !value.has_expired_after(self.client.discovery_cache_timeout())
                {
...
mod tests {
    ...
    use super::*;
    use crate::{base::sleep::sleep, test::async_test, test_utils::mocks::MatrixMockServer};
