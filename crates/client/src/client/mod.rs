...
pub(crate) struct ClientInner {
    ...
    /// [`crate::common::cross_process_lock::CrossProcessLock`]) when
...
impl Client {
    ...
    ///
    /// The SDK provides cross-process store locks (see
    /// [`crate::common::cross_process_lock::CrossProcessLock`]) when this
    ...
    /// # let mut client = Client::new(homeserver).await?;
    /// use crate::ruma::{api::client::profile, owned_user_id};
    ///
    ...
    ///
    /// ```no_run
    /// use crate::ruma::api::FeatureFlag;
    /// # use crate::{Client, config::SyncSettings};
    /// # use url::Url;
    /// # async {
    /// # let homeserver = Url::parse("http://localhost:8080")?;
    ...
    ///
    /// ```no_run
    /// # use crate::{Client, ruma::api::client::authenticated_media::get_media_preview};
    ...
    /// # Examples
    /// ```no_run
    /// # use crate::{Client, config::SyncSettings, ruma::api::client::rtc::RtcTransport};
    /// # use url::Url;
    /// # async {
    /// # let homeserver = Url::parse("http://localhost:8080")?;
    /// # let mut client = Client::new(homeserver).await?;
    ...
    /// # use crate::{Client, config::SyncSettings, LoopCtrl};
    /// # use url::Url;
    /// # async {
    /// # let homeserver = Url::parse("http://localhost:8080").unwrap();
    /// # let mut client = Client::new(homeserver).await.unwrap();
    ...
    /// `crate::ui::sync_service::SyncService`.
    ...
    ///
    /// If your app stopped the `crate::ui::sync_service::SyncService`
...
pub(crate) mod tests {
    ...
    async fn test_uploading_a_too_large_media_file() {
        ...
        let data = vec![1, 2];
        let upload_request =
            harana_matrix_common::api::client::media::create_content::v3::Request::new(
                data.clone(),
            );
    ...
    async fn test_threading_support_reflects_the_builder_configuration() {
        use crate::base::ThreadingSupport;
