...
use crate::base::crypto::DecryptionSettings;
#[cfg(feature = "experimental-x509-identity-verification")]
use crate::base::crypto::x509::{RawX509Signer, RawX509Verifier};
#[cfg(feature = "e2e-encryption")]
use crate::base::crypto::{CollectStrategy, TrustRequirement};
#[cfg(feature = "e2e-encryption")]
use crate::encryption::EncryptionSettings;
#[cfg(all(not(target_family = "wasm"), feature = "reqwest-transport"))]
use crate::http_client::HttpSettings;
#[cfg(feature = "experimental-search-core")]
use crate::search::backend::SearchIndexProvider;
#[cfg(feature = "experimental-search-core")]
use crate::search_index::SearchIndex;
#[cfg(feature = "experimental-search-core")]
use crate::search_index::{SearchIndexStoreKind, default_search_index_provider};
#[cfg(feature = "sqlite")]
use crate::sqlite::SqliteStoreConfig;
use crate::{
    HttpError, IdParseError,
    authentication::AuthCtx,
    base::{BaseClient, DmRoomDefinition, ThreadingSupport, store::StoreConfig, ttl::TtlValue},
...
impl ClientBuilder {
    ...
    /// [`crate::common::cross_process_lock::CrossProcessLock`]). The
    ...
    #[instrument(skip_all, target = "crate::client", fields(homeserver))]
    ...
    let store_config = match builder_config {
        ...
        BuilderStoreConfig::Sqlite { config, cache_path } => {
            ...
            let (state_store, event_cache_store, media_store, crypto_store) = try_join!(
                crate::sqlite::SqliteStateStore::open_with_config(&config),
                crate::sqlite::SqliteEventCacheStore::open_with_config(&config_with_cache_path),
                crate::sqlite::SqliteMediaStore::open_with_config(&config_with_cache_path),
                crate::sqlite::SqliteCryptoStore::open_with_config(&config),
            )?;
            #[cfg(not(feature = "e2e-encryption"))]
            let (state_store, event_cache_store, media_store) = try_join!(
                crate::sqlite::SqliteStateStore::open_with_config(&config),
                crate::sqlite::SqliteEventCacheStore::open_with_config(&config_with_cache_path),
                crate::sqlite::SqliteMediaStore::open_with_config(&config),
            )?;
...
async fn build_indexeddb_store_config(
    ...
    let stores = crate::indexeddb::IndexeddbStores::open(name, passphrase).await?;
...
pub enum ClientBuildError {
    ...
    #[cfg(feature = "indexeddb")]
    #[error(transparent)]
    IndexeddbStore(#[from] crate::indexeddb::OpenStoreError),

    /// Error opening the sqlite store.
    #[cfg(feature = "sqlite")]
    #[error(transparent)]
    SqliteStore(#[from] crate::sqlite::OpenStoreError),
...
pub(crate) mod tests {
    ...
    impl StoreProvider for TestStoreProvider {
        fn open_stores<'a>(
            &'a self,
            cross_process_lock_config: &'a CrossProcessLockConfig,
        ) -> crate::common::BoxFuture<'a, Result<StoreConfig, StoreProviderError>> {
            Box::pin(async move {
