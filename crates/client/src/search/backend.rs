...
use crate::search::error::IndexError;
...
pub struct IndexableEvent {
    ...
    ///
    /// Please use the `crate::common::TimelineEvent::timestamp` as much as
    /// possible as it protects against malformed `origin_server_ts`. At worst,
    /// use the `crate::common::serde_helpers::extract_timestamp` function.
