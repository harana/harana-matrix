...
//! ```
//!
//! [`OlmMachine`]: /crypto/struct.OlmMachine.html
...
use harana_matrix_common::{
    ...
    olm::{Curve25519PublicKey, megolm::SessionOrdering},
};
...
#[cfg(doc)]
use crate::crypto::{backups::BackupMachine, identities::OwnUserIdentity};
...
use crate::crypto::x509::{X509Signer, X509Verifier};
...
use crate::{
    common::{
        SyncOutsideWasm,
        cross_process_lock::{
            CrossProcessLock, CrossProcessLockConfig, CrossProcessLockGeneration,
        },
...
impl SyncedKeyQueryManager<'_> {
    ...
    /// See the docs for [`crate::crypto::OlmMachine::tracked_users()`].
...
impl Store {
    ...
    pub(crate) async fn with_transaction<T>(
        &self,
        func: impl AsyncFnOnce(&mut StoreTransaction) -> Result<T, crate::crypto::OlmError>,
    ) -> Result<T, crate::crypto::OlmError> {
    ...
    /// ```no_run
    /// # use std::io::Cursor;
    /// # use crate::crypto::{OlmMachine, decrypt_room_key_export};
    /// # use harana_matrix_common::{device_id, user_id};
    ...
    ///
    /// ```no_run
    /// # use crate::crypto::{OlmMachine, encrypt_room_key_export};
    ...
    ///
    /// use crate::crypto::{OlmMachine, olm::ExportedRoomKey};
    /// # async {
    /// let alice = user_id!("@alice:example.org");
    /// let machine = OlmMachine::new(&alice, device_id!("DEVICEID")).await;
    /// let room_id = room_id!("!test:localhost");
...
impl crate::common::cross_process_lock::TryLock for LockableCryptoStore {
...
mod tests {
    ...
    use harana_matrix_common::{
        ...
        olm::{Ed25519Keypair, megolm::SessionKey},
    ...
    fn make_inbound_group_session(
        ...
        InboundGroupSession::new(
            ...
            SenderData::device_info(crate::crypto::types::DeviceKeys::new(
                sender_account.user_id().to_owned(),
                sender_account.device_id().to_owned(),
                vec![],
                BTreeMap::new(),
                crate::crypto::types::Signatures::new(),
            )),
            None,
