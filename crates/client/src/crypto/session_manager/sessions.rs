...
use crate::{
    common::{failures_cache::FailuresCache, locks::RwLock as StdRwLock},
    crypto::{
        DeviceData,
        error::OlmResult,
...
pub(crate) struct SessionManager {
    ...
    /// users/devices.
    ///
    /// According to the doc on
    /// [`crate::crypto::OlmMachine::get_missing_sessions`], there should
    /// only be one such request active at a time, so we only need to keep a
    /// record of the most recent.
    ...
    /// `/keys/claim` response.
    ///
    /// See also [`crate::crypto::identities::IdentityManager::failures`].
...
mod tests {
    ...
    use crate::{
        common::{executor::spawn, locks::RwLock as StdRwLock},
        crypto::{
            gossiping::GossipMachine,
