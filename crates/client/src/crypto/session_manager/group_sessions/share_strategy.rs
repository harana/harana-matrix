...
use crate::crypto::x509::X509Verifier;
#[cfg(doc)]
use crate::crypto::{Device, UserIdentity};
use crate::{
    common::deserialized_responses::WithheldCode,
    crypto::{
        DeviceData, EncryptionSettings, LocalTrust, OlmError, OwnUserIdentityData,
        UserIdentityData,
...
mod tests {
    ...
    use crate::{
        common::deserialized_responses::WithheldCode,
        crypto::{
            CrossSigningKeyExport, DeviceData, EncryptionSettings, LocalTrust, OlmError,
            OlmMachine,
    ...
    async fn get_own_identity_data(
        machine: &OlmMachine,
        user_id: &UserId,
    ) -> Option<crate::crypto::OwnUserIdentityData> {
        machine
    ...
    async fn get_user_identity_data(
        machine: &OlmMachine,
        user_id: &UserId,
    ) -> Option<crate::crypto::UserIdentityData> {
        use crate::crypto::{UserIdentity, identities::user::UserIdentityData};
    ...
    mod dehydrated_device {
        ...
        use harana_matrix_common::{
            DeviceId, TransactionId, UserId, device_id,
            olm::{Curve25519PublicKey, Ed25519SecretKey},
            user_id,
        };
        ...
        async fn should_give_error_for_dehydrated_device_of_verification_violation_user(
            ...
            assert_matches::assert_matches!(
                share_result,
                Err(crate::crypto::OlmError::SessionRecipientCollectionError(
                    crate::crypto::SessionRecipientCollectionError::VerifiedUserChangedIdentity(_)
                ))
            );
        }
    ...
    async fn test_share_identity_strategy_report_verification_violation() {
        ...
        machine
            .mark_request_as_sent(
                &TransactionId::new(),
                crate::crypto::types::requests::AnyIncomingResponse::KeysQuery(&kq_response),
            )
            .await
            .unwrap();
    ...
    async fn test_should_rotate_based_on_device_with_pending_request_excluded() {
...

        {
            ...
            let req = Arc::new(ToDeviceRequest::for_recipients(
                ...
                &harana_matrix_common::events::AnyToDeviceEventContent::Dummy(
                    ToDeviceDummyEventContent,
                ),
                txid.clone(),
            ));
