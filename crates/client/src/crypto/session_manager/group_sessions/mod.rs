...
#[cfg(feature = "experimental-encrypted-state-events")]
use crate::crypto::types::events::room::encrypted::RoomEncryptedEventContent;
use crate::{
    common::{deserialized_responses::WithheldCode, executor::spawn, locks::RwLock as StdRwLock},
    crypto::{
...
mod tests {
    ...
    use crate::{
        common::deserialized_responses::{ProcessedToDeviceEvent, WithheldCode},
        crypto::{
        ...
        test::{async_test, ruma_response_from_json},
    };
    ...
    fn keys_query_response() -> get_keys::v3::Response {
        let data =
            include_bytes!("../../../../../../benchmarks/benches/crypto_bench/keys_query.json");
    ...
    fn keys_claim_response() -> claim_keys::v3::Response {
        let data =
            include_bytes!("../../../../../../benchmarks/benches/crypto_bench/keys_claim.json");
        let data: Value = serde_json::from_slice(data).unwrap();
        ruma_response_from_json(&data)
    }
    ...
    async fn test_resend_session_after_unwedging() {
...

        {
            ...
            let sync_changes = EncryptionSyncChanges {
                to_device_events: vec![crate::crypto::utilities::json_convert(&to_device).unwrap()],
    ...
    async fn test_room_key_bundle_sharing() {
        ...
        assert_eq!(1, decrypted.len());
        use crate::crypto::types::events::EventType;
        assert_let!(
    ...
    async fn test_room_key_bundle_sharing_sends_no_olm_withheld() {
        ...
        alice
            .store()
            .save_changes(crate::crypto::store::types::Changes {
                devices: crate::crypto::store::types::DeviceChanges {
                    new: vec![unreachable],
                    ..Default::default()
                },
                ..Default::default()
