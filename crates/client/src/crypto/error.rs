...
#[cfg(doc)]
use crate::crypto::{CollectStrategy, Device, LocalTrust, OtherUserIdentity};
use crate::{
    common::deserialized_responses::{VerificationLevel, WithheldCode},
    crypto::{olm::SessionExportError, types::SignedKey},
};
...
pub enum MegolmError {
    ...
    /// The event was redacted before we saw it, so there is nothing to
    /// decrypt.
    ///
    /// A redaction strips the `algorithm` and `ciphertext` fields, which makes
    /// the event indistinguishable from one using an algorithm we don't know
...
pub enum SignatureError {
    ...
    X509SigningError(crate::crypto::x509::X509SignatureSigningError),
...
/// [`crate::crypto::machine::OlmMachine::set_room_settings`].
#[derive(Debug, Error)]
