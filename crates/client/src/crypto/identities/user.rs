...
impl OwnUserIdentity {
    ...
    pub async fn verify(&self) -> Result<SignatureUploadRequest, SignatureError> {
        ...
        // N.B. Duplicate of
        // `crate::crypto::olm::signing::PrivateCrossSigningIdentity::for_account`
...
impl OtherUserIdentityData {
    ...
    #[cfg(test)]
    pub(crate) async fn from_private(
        identity: &crate::crypto::olm::PrivateCrossSigningIdentity,
    ) -> Self {
...
pub(crate) mod testing {
    ...
    #[cfg(test)]
    use crate::crypto::{
        identities::manager::testing::other_user_id, olm::PrivateCrossSigningIdentity,
    };
    use crate::{
        crypto::{
            identities::{
    ...
    ///         &TransactionId::new(),
    ///         crate::crypto::IncomingResponse::KeysQuery(&kq_response),
    ///     )
    ///     .await
    ///     .unwrap();
    /// ```
...
pub(crate) mod tests {
    ...
    use crate::{
        crypto::{
            ...
            types::{
                CrossSigningKey, MasterPubkey, SelfSigningPubkey, Signatures, UserSigningPubkey,
            },
            verification::VerificationMachine,
        },
    ...
    async fn test_refresh_signature() {
        ...
        let (x509_signer_old, x509_signer_current, x509_signer_new) =
            crate::crypto::x509::tests::signers_with_different_validity();
    ...
    async fn create_store_with_private_identity_and_x509(
        ...
        use crate::crypto::store::types::{Changes, IdentityChanges, PendingChanges};
