...
use crate::crypto::x509::X509Signer;
use crate::crypto::{
...
impl PrivateCrossSigningIdentity {
    ...
    pub(crate) async fn for_account(
        ...
        // `crate::crypto::identities::user::OwnUserIdentity::verify`,
...
mod tests {
    ...
    use harana_matrix_common::{
        CanonicalJsonValue, DeviceKeyAlgorithm, DeviceKeyId, UserId, device_id, user_id,
    };
    use serde_json::json;
    ...
    async fn test_private_identity_signed_by_x509() {
        use crate::crypto::x509::tests::create_rust_signer_and_verifier;

        let account = Account::with_device_id(user_id(), device_id!("DEVICEID"));
        let (cert, signing_key) =
            crate::crypto::x509::tests::cert_and_key_with_email_in_subject_distinguished_name(
                "example@localhost",
            );
