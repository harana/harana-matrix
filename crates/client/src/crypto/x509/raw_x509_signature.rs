...
use harana_matrix_common::{OwnedDeviceId, olm::base64_encode};
...
impl TryFrom<SignedData> for RawX509SignatureAndFirstCertificate {
    ...
    fn try_from(data: SignedData) -> Result<Self, Self::Error> {
        ...
        // Get the signer info and check there is only one.
...
/// Check that the given [`EncapsulatedContentInfo`] is empty and has the
/// correct OID.
...
/// them to one of our supported [`X509SignatureScheme`]s. Per the (draft)
/// specification, `signatureAlgorithm` MUST represent RSASSA-PSS with SHA512,
/// and a salt length of 64, as follows:
///
/// ```text
/// signatureAlgorithm:
...
/// [`crate::crypto::x509`] module documentation.
