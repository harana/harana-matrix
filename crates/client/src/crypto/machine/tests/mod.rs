...
use crate::{
    ...
    crypto::{
        Account, DecryptionSettings, Device, DeviceData, EncryptionSettings, LocalTrust,
        MegolmError, OlmError, OlmMachineBuilder, RoomEventDecryptionResult, TrustRequirement,
...
/// [`MemoryStore`](crate::crypto::store::MemoryStore) completes without waiting
/// for anything, so a future that is still pending after this is one that is
/// waiting on a lock.
...
async fn test_query_ratcheted_key() {
    ...
    if let MegolmError::Decryption(vodo_error) = decrypt_error {
        if let harana_matrix_common::olm::megolm::DecryptionError::UnknownMessageIndex(_, _) =
            vodo_error
        {
...
async fn test_importing_private_cross_signing_keys_verifies_the_public_identity() {
    async fn create_additional_machine(machine: &OlmMachine) -> OlmMachine {
        ...
        changes.identities.new.push(crate::crypto::UserIdentityData::Own(identity.inner));
...
async fn test_retry_decryption_of_bundled_events() {
    ...
    let edit_content = RoomMessageEventContent::text_plain("edited").make_replacement(
        harana_matrix_common::events::room::message::ReplacementMetadata::new(
...
async fn test_mark_all_tracked_users_as_dirty() {
    ...
    crate::crypto::store::migrations::mark_all_data_migrations_as_done(&store).await.unwrap();
