...
use harana_matrix_common::{
    DeviceKeyId, RoomId, TransactionId, UserId, canonical_json::to_canonical_value,
    olm::Curve25519SecretKey, owned_user_id, room_id, user_id,
};
use serde::Serialize;
use serde_json::json;
...
use crate::{
    common::{SyncOutsideWasm, deserialized_responses::ProcessedToDeviceEvent},
    crypto::{
        Account, DecryptionSettings, DeviceData, EncryptionSettings, EncryptionSyncChanges,
        EventError, OlmError, OlmMachine, Session, TrustRequirement,
...
async fn test_update_unknown_device_senderdata_on_keys_query() {
    ...
    bob.receive_keys_query_response(
        &TransactionId::new(),
        &crate::test::ruma_response_from_json(&kq_response),
    )
    .await
    .unwrap();
...
async fn forget_devices_for_user(machine: &OlmMachine, other_user: &UserId) {
    let mut keys_query_response =
        harana_matrix_common::api::client::keys::get_keys::v3::Response::default();
