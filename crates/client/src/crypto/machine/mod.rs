...
use harana_matrix_common::{
    ...
    olm::{Curve25519PublicKey, Ed25519Signature, megolm::DecryptionError},
...
use crate::common::deserialized_responses::WithheldCode;
#[cfg(feature = "experimental-push-secrets")]
use crate::crypto::error::SecretPushError;
#[cfg(feature = "experimental-send-custom-to-device")]
use crate::crypto::session_manager::split_devices_for_share_strategy;
#[cfg(feature = "experimental-x509-identity-verification")]
use crate::crypto::x509::{RawX509Signer, RawX509Verifier, X509Signer, X509Verifier};
use crate::{
    common::{
        BoxFuture,
    ...
    crypto::{
        ...
        store::{
            CryptoStoreWrapper, DynCryptoStore, IntoCryptoStore, MemoryStore,
            Result as StoreResult, SecretImportError, Store, StoreTransaction,
            caches::StoreCache,
            types::{
        ...
        types::{
            ...
            events::{
                ...
                olm_v1::{
                    AnyDecryptedOlmEvent, DecryptedRoomKeyBundleEvent, DecryptedRoomKeyEvent,
                },
                room::encrypted::{
...
impl OlmMachineBuilder {
    ...
    ///
    /// [`CryptoStore`]: crate::crypto::store::CryptoStore
    pub fn with_crypto_store(mut self, store: impl IntoCryptoStore) -> Self {
        self.store = Some(store.into_crypto_store());
        self
    }

    /// Set a custom [`harana_matrix_common::olm::olm::Account`] to be used for
    /// the identity and one-time keys of this [`OlmMachine`]. If this is
    /// not set before [`OlmMachineBuilder::build`] is called, a new default
    /// one or one from the store will be used.
    ///
...
impl OlmMachine {
    ...
    pub(crate) async fn from_builder(builder: OlmMachineBuilder) -> StoreResult<Self> {
        ...
        // per backend.
        crate::crypto::store::migrations::run_data_migrations(
            &crate::crypto::store::migrations::DataMigrationContext {
                store: &store,
                identity_manager: &identity_manager,
            },
            &crate::crypto::store::migrations::builtin_data_migrations(),
        )
        .await?;
    ...
    async fn handle_to_device_event(&self, changes: &mut Changes, event: &ToDeviceEvents) {
        use crate::crypto::types::events::ToDeviceEvents::*;
