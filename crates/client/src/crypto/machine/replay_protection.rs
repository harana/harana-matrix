...
/// [`OlmMachine`](crate::crypto::OlmMachine). A replayed message that arrives
/// in a later process is not detected, but the copy that is kept and shown is
/// then the first one seen in that process, so the timeline still holds one
/// copy of the message rather than two.
#[derive(Debug, Default)]
