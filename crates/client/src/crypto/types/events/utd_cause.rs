...
pub enum UtdCause {
    ...
    /// The [`crate::crypto::TrustRequirement`] requires that the sending device
    /// be signed by its owner, and it was not.
    UnsignedDevice = 3,

    /// The [`crate::crypto::TrustRequirement`] requires that the sending device
    /// be signed by its owner, and we were unable to securely find the
    /// device.
    ///
...
mod tests {
    use harana_matrix_common::{
        MilliSecondsSinceUnixEpoch, events::AnySyncTimelineEvent, serde::Raw,
    };
    ...
    use crate::{
        ...
        crypto::types::events::{UtdCause, utd_cause::CryptoContextInfo},
    };
