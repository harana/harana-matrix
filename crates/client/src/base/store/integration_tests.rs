...
use crate::{
    ...
    common::ttl::TtlValue,
    test::{TestResult, event_factory::EventFactory},
};
...
impl StateStoreIntegrationTests for DynStateStore {
    async fn populate(&self) -> TestResult {
        ...
        room.handle_state_event(
            &mut RawStateEventWithKeys::try_from_raw_state_event(name_raw.clone())
        ...
        room.handle_state_event(
            &mut RawStateEventWithKeys::try_from_raw_state_event(topic_raw.clone())
                .expect("generated state event should be valid"),
        );
...
macro_rules! statestore_integration_tests {
    () => {
        mod statestore_integration_tests {
            use $crate::base::store::{IntoStateStore, StateStoreIntegrationTests};

            use super::get_store;
            use crate::test::{TestResult, async_test};
