...
use crate::{
    base::{RoomInfo, StateChanges, StateStore, store::BaseStateStore},
    timer,
};
...
mod tests {
    ...
    async fn process(client: &crate::base::BaseClient, events: &[Raw<AnyGlobalAccountDataEvent>]) {
    ...
    fn direct_event(pairs: &[(&str, &[&str])]) -> Raw<AnyGlobalAccountDataEvent> {
        ...
        for (user_id, room_ids) in pairs {
            ...
            for room_id in *room_ids {
                builder = builder.add_user(
                    user_id.clone(),
                    <&harana_matrix_common::RoomId>::try_from(*room_id).unwrap(),
                );
            }
        }
