...
#[cfg(feature = "e2e-encryption")]
use crate::base::RoomMemberships;
#[cfg(feature = "experimental-x509-identity-verification")]
use crate::crypto::x509::{RawX509Signer, RawX509Verifier};
#[cfg(feature = "e2e-encryption")]
use crate::crypto::{
...
use crate::{
    ...
    common::{cross_process_lock::CrossProcessLockConfig, timer},
};
...
pub struct BaseClient {
    ...
    /// [`SessionMeta`][crate::base::session::SessionMeta] is set via
    /// [`BaseClient::activate`]
    #[cfg(feature = "e2e-encryption")]
...
impl BaseClient {
    ...
    ///
    /// * `custom_account` - A custom [`crate::crypto::vodozemac::olm::Account`]
    ///   to be used for the identity and one-time keys of this [`BaseClient`].
    ///   If no account is provided, a new default one or one from the store
    ///   will be used. If an account is provided and one already exists in the
    ///   store for this [`UserId`]/[`DeviceId`] combination, an error will be
    ///   raised. This is useful if one wishes to create identity keys before
    ///   knowing the user/device IDs, e.g., to use the identity key as the
    ///   device ID.
    ///
    ...
    pub async fn activate(
        ...
        #[cfg(feature = "e2e-encryption")] custom_account: Option<
            crate::base::crypto::vodozemac::olm::Account,
        >,
    ) -> Result<()> {
    ...
    pub async fn regenerate_olm(
        &self,
        custom_account: Option<crate::base::crypto::vodozemac::olm::Account>,
    ) -> Result<()> {
    ...
    ///
    /// ```rust
    /// # use crate::base::{BaseClient, store::StoreConfig, RoomState, ThreadingSupport, DmRoomDefinition};
    ...
    pub async fn receive_sync_response_with_requested_required_states(
        ...
        let to_device = response
            ...
            .into_iter()
            .map(|raw| {
                use crate::common::deserialized_responses::{
...
mod tests {
    ...
    #[cfg(feature = "unstable-msc4426")]
    use crate::base::{RoomMemberships, store::StateChanges};
    use crate::{
        base::{
            DmRoomDefinition, RoomDisplayName, RoomInfoNotableUpdateReasons, RoomState,
            SessionMeta,
            client::ThreadingSupport,
            store::{
                RoomLoadSettings, StateStoreDataKey, StateStoreDataValue, StateStoreExt,
                StoreConfig,
            },
    ...
    async fn test_receive_sent_state_event_makes_it_readable_right_away() {
        ...
        let response = sync_builder
            .add_joined_room(crate::test::JoinedRoomBuilder::new(room_id))
            .build_sync_response();
    ...
    async fn test_reinvited_members_get_a_display_name() {
        ...
        let response = sync_builder
            .add_joined_room(
                crate::test::JoinedRoomBuilder::new(room_id)
    ...
    async fn test_room_member_carries_global_profile_status() {
        ...
        let response = sync_builder
            .add_joined_room(
                crate::test::JoinedRoomBuilder::new(room_id).add_state_event(f.member(user_id)),
            )
            .build_sync_response();
        client.receive_sync_response(response).await.unwrap();
