...
use crate::common::deserialized_responses::ProcessedToDeviceEvent;
use crate::{
    base::{
            ...
            for room in self.state_store.rooms() {
                    ...
                    .any(|user_id| extensions.profiles.users.contains_key(user_id))
                {
                    room.update_room_info_with_store_guard(state_store_guard, |room_info| {
                        (room_info, crate::base::RoomInfoNotableUpdateReasons::HEROES)
                    })
                    .map_err(crate::base::StoreError::from)?;
                }
            }
        }
