...
However, the crate is designed in a modular way, and its lower layers are
usable on their own. If you're attempting something more custom, you might be
interested in these:

- [`base`]: A no-network-IO client state machine which can be used to embed a
  Matrix client into an existing network stack or to build a new Matrix client
  library on top.
- [`crypto`]: A no-network-IO encryption state machine which can be used to add
  Matrix E2EE support into an existing client or library.
...
- **Spawning tasks, sleeping and running blocking work.** This is described by
  [`common::runtime::AsyncRuntime`]. Tokio is used by default (the
  `tokio-runtime` feature); install your own with
  [`common::runtime::set_runtime`] before creating a [`Client`].
