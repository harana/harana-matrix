...
use harana_matrix_client::{
    ...
    test::{BOB, JoinedRoomBuilder, async_test, event_factory::EventFactory},
    test_utils::mocks::{
        MatrixMockServer, RoomMessagesResponseTemplate, RoomRelationsResponseTemplate,
    },
    timeout::timeout,
    ui::timeline::{RoomExt, TimelineBuilder, TimelineFocus},
};
...
impl PinnedEventsSync {
    ...
    async fn mock_and_sync(
        ...
        client: &Client,
        server: &MatrixMockServer,
    ) -> Result<Room, harana_matrix_client::Error> {
