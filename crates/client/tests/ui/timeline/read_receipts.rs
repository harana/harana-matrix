...
use harana_matrix_client::{
    ...
    ui::timeline::{RoomExt, TimelineFocus, TimelineReadReceiptTracking},
};
...
async fn wait_for_empty_send_queue(
    client: &harana_matrix_client::Client,
    room_id: &harana_matrix_common::RoomId,
) {
    for _ in 0..300 {
...
async fn test_no_duplicate_receipt_after_backpagination() {
    ...
    let read_receipt_event = f
        ...
        .add(
            eid3,
            *CAROL,
            harana_matrix_common::events::receipt::ReceiptType::Read,
            ReceiptThread::Unthreaded,
        )
        .into_event();
