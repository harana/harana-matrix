...
use harana_matrix_client::{
    ...
    test::{JoinedRoomBuilder, SyncResponseBuilder, TestResult, async_test},
    test_utils::{
...
use harana_matrix_common::{
    ...
    olm::{
        Curve25519PublicKey, Curve25519SecretKey, Ed25519PublicKey, Ed25519SecretKey,
        olm::IdentityKeys,
    },
...
async fn test_incremental_upload_of_keys_sliding_sync() -> TestResult {
    ...
    let sliding =
        client
            ...
            .add_list(harana_matrix_client::SlidingSyncList::builder("all").sync_mode(
                harana_matrix_client::SlidingSyncMode::new_selective().add_range(0..=20),
            ))
            .build()
            .await?;
...
async fn test_a_backed_up_room_key_we_cannot_read_is_reported() -> TestResult {
    ...
    assert_matches!(
        error,
        harana_matrix_client::Error::CorruptBackupRoomKey { session_id: reported } => {
...
-> TestResult {
    ...
    let outbound_group_session = OutboundGroupSession::new(
        ...
        room_id,
        harana_matrix_client::base::crypto::EncryptionSettings::default(),
    )?;
