...
async fn test_restoring_a_session_uses_the_homeserver_stored_with_it() {
    ...
    client
        .matrix_auth()
        .restore_session(session, harana_matrix_client::store::RoomLoadSettings::default())
        .await
        .unwrap();
...
async fn test_restore_session_with_access_token_without_a_device_id() {
    ...
    assert_matches!(error, harana_matrix_client::Error::MissingDeviceId);
    assert!(!client.matrix_auth().logged_in());
}
...
async fn test_login_with_cross_signing_bootstrapping() {
...

    {
        ...
        let client = Client::builder()
            ...
            .with_encryption_settings(harana_matrix_client::encryption::EncryptionSettings {
...
async fn test_login_with_cross_signing_bootstrapping_already_bootstrapped() {
    ...
    let (builder, server) =
        harana_matrix_client::test_utils::test_client_builder_with_server().await;
