...
use harana_matrix_client::attachment::{GalleryConfig, GalleryItemInfo};
use harana_matrix_client::{
    ...
    test::{
        ALICE, InvitedRoomBuilder, JoinedRoomBuilder, KnockedRoomBuilder, LeftRoomBuilder,
...
async fn test_retry_requests_blocked_on_verification() {
    use harana_matrix_client::base::store::QueueWedgeError;
...
async fn test_reloading_rooms_with_unsent_events() {
    ...
    let client = mock
        ...
        .on_builder(|builder| {
            builder.store_config(
                StoreConfig::new(CrossProcessLockConfig::SingleProcess).state_store(store.clone()),
            )
        })
        .build()
        .await;
    ...
    let new_client = mock
        ...
        .on_builder(|builder| {
            builder.store_config(
                StoreConfig::new(CrossProcessLockConfig::SingleProcess).state_store(store),
            )
        })
        .build()
        .await;
...
async fn test_media_uploads() {
    ...
    let config = AttachmentConfig::new()
        ...
        .reply(Some(Reply {
            ...
            enforce_thread: harana_matrix_client::room::reply::EnforceThread::Threaded(
