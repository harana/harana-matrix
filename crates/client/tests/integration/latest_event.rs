...
async fn test_latest_event_is_recomputed_when_a_user_is_ignored() {
    ...
    // Bob's event is dropped, but Alice's event is kept: it becomes the latest
    // event of the room, instead of the room losing its latest event entirely.
    assert_matches!(
...
#[async_test]
async fn test_an_event_from_an_ignored_user_is_not_a_latest_event() {
    let room_id = owned_room_id!("!r0");
    let alice = user_id!("@alice:local");
    let bob = user_id!("@bob:local");
    let event_alice = event_id!("$ev0");
    ...
    // Fill the event cache with two events: the most recent one is Bob's.
    client
        .event_cache_store()
        .lock()
        .await
    ...
    // Bob is ignored before the room, and thus its cache, is known.
    server
        .mock_sync()
        .ok_and_run(&client, |sync_builder| {
            sync_builder.add_global_account_data(event_factory.ignored_user_list([bob.to_owned()]));
