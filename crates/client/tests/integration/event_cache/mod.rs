...
use harana_matrix_client::{
    ...
    store::StoreConfig,
    test::{ALICE, BOB, JoinedRoomBuilder, async_test, event_factory::EventFactory},
    test_utils::{
...
async fn test_apply_redaction_when_redaction_comes_later() {
    ...
    // clients.
    let state_memory_store = harana_matrix_client::base::store::MemoryStore::new();
    ...
    server
        .sync_room(
            ...
            JoinedRoomBuilder::new(room_id)
                ...
                .add_timeline_bulk(
                    latest_events.clone().into_iter().map(harana_matrix_common::serde::Raw::cast),
                ),
        )
        .await;
    ...
    server
        .sync_room(
            ...
            JoinedRoomBuilder::new(room_id)
                ...
                .add_timeline_bulk(
                    sync_events.into_iter().map(harana_matrix_common::serde::Raw::cast),
                ),
        )
        .await;
