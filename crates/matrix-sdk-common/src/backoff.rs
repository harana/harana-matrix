...
//
// Ported from tuwunel `src/core/utils/time/exponential_backoff.rs`.
...
//!
//! The window grows quadratically with the retry count: the hold after `tries`
//! failures is `min * tries^2`, capped at `max`.
...
///
/// The window grows quadratically with the retry count and is capped at `max`.
/// `None` means the hold has elapsed.
#[inline]
#[must_use]
pub fn exponential_backoff_remaining(
    ...
    let window = min.saturating_mul(tries).saturating_mul(tries).min(max);

    window.checked_sub(elapsed).filter(|remaining| !remaining.is_zero())
}
...
///
/// Beyond this many tries the window is pinned at `max`, so a caller counting
/// retries has no reason to keep incrementing.
#[inline]
#[must_use]
pub fn exponential_backoff_streak_cap(min: Duration, max: Duration) -> u32 {
    ...
    let ceil = if floor.saturating_mul(floor) < ratio { floor.saturating_add(1) } else { floor };
...
mod tests {
    ...
    #[test]
    fn test_remaining_tracks_the_quadratic_window() {
        let min = Duration::from_secs(3);
        let max = Duration::from_secs(60);
        let elapsed = Duration::from_secs(5);

        // 3s * 2 * 2 = 12s of hold, 5s of which has elapsed.
        assert_eq!(
    ...
    #[test]
    fn test_remaining_caps_and_expires_at_the_boundary() {
        let min = Duration::from_secs(3);
        let max = Duration::from_secs(60);

        // The quadratic window would be far past `max`, so the cap applies.
        assert_eq!(
            exponential_backoff_remaining(min, max, Duration::from_secs(15), 100),
            Some(Duration::from_secs(45)),
        );

        assert_eq!(exponential_backoff_remaining(min, max, Duration::from_secs(60), 100), None);
        assert_eq!(
    ...
    #[test]
    fn test_remaining_secs_matches_the_duration_form() {
        assert_eq!(
            exponential_backoff_remaining_secs(3, 60, Duration::from_secs(5), 2),
    ...
    #[test]
    fn test_streak_cap_is_the_ceiling_of_the_root_ratio() {
        // 60 / 3 = 20, and ceil(sqrt(20)) is 5.
        assert_eq!(
            exponential_backoff_streak_cap(Duration::from_secs(3), Duration::from_secs(60)),
            5
        );
        // 36 / 4 = 9, an exact square.
        assert_eq!(
            exponential_backoff_streak_cap(Duration::from_secs(4), Duration::from_secs(36)),
            3
        );
        // A max below min still allows one try.
        assert_eq!(
            exponential_backoff_streak_cap(Duration::from_secs(60), Duration::from_secs(3)),
            1
        );
    }
}
