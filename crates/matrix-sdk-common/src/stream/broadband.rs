...
//
// Ported from tuwunel `src/core/utils/stream/broadband.rs`.

//! Broadband stream combinator extensions to `futures_util::Stream`.
...
/// become ready rather than in source order. Use [`super::WidebandExt`] when
    ...
    S: Stream<Item = Item> + Send + Sized,
{
    ...
    where
        ...
        Fut: Future<Output = bool> + Send,
    {
        self.map(f).buffer_unordered(n.into().unwrap_or_else(automatic_width)).ready_all(identity)
    }
    ...
    where
        ...
        Fut: Future<Output = bool> + Send,
    {
        self.map(f).buffer_unordered(n.into().unwrap_or_else(automatic_width)).ready_any(identity)
    }
    ...
    where
        ...
        U: Send,
    {
        self.map(f).buffer_unordered(n.into().unwrap_or_else(automatic_width))
    }
}
