...
//
// Ported from tuwunel `src/core/utils/stream/ready.rs`.

//! Synchronous combinator extensions to `futures_util::Stream`.
    ...
    S: Stream<Item = Item> + Sized,
{
    ...
    where
        ...
        Item: Send,
    {
        self.ready_filter(f).take(1).into_future().map(|(curr, _next)| curr)
    }
    ...
    where
        ...
        U: Send,
    {
        self.ready_filter_map(f).take(1).into_future().map(|(curr, _next)| curr)
    }
