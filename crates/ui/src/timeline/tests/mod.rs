...
impl PaginableThread for TestRoomDataProvider {
    async fn relations(
        ...
        _opts: matrix::room::RelationsOptions,
    ) -> Result<Relations, matrix::Error> {
        unimplemented!();
    }
...
impl RoomDataProvider for TestRoomDataProvider {
    ...
    async fn load_event<'a>(&'a self, event_id: &'a EventId) -> matrix::Result<TimelineEvent> {
        self.loaded_events.write().await.push(event_id.to_owned());
        self.loadable_events
            .get(event_id)
            .cloned()
            .ok_or_else(|| matrix::Error::UnknownError("event not found".into()))
    }
    ...
    ) -> matrix::Result<(TimelineEvent, Vec<TimelineEvent>)> {
        unimplemented!();
    }
}
