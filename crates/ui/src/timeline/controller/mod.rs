...
impl<P: RoomDataProvider> TimelineController<P> {
    ...
    /// [`base::RoomInfo`] and applies the new value to the
    ...
    pub(super) async fn set_sender_profiles_error(&self, error: Arc<matrix::Error>) {
                ...
                if let Some(send_error) = send_error {
                    self.update_event_send_state(
                        ...
                        EventSendState::SendingFailed {
                            error: Arc::new(matrix::Error::SendQueueWedgeError(Box::new(
                                send_error,
                            ))),
