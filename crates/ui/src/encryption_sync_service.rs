...
impl EncryptionSyncService {
    ...
    ///
    /// This will create and manage an instance of [`matrix::SlidingSync`].
    pub async fn new(
        client: Client,
    ...
    async fn new_inner(
            ...
            client.cross_process_lock_config()
        {
            ...
            match client.encryption().enable_cross_process_store_lock(holder_name.clone()).await {
                Ok(()) | Err(matrix::Error::BadCryptoStoreState) => {
...
pub enum Error {
    ...
    SlidingSync(matrix::Error),

    #[error("Locking failed: {0:#}")]
    LockError(matrix::Error),

    #[error(transparent)]
    ClientError(matrix::Error),
}
