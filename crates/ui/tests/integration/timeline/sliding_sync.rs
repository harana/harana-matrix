                ...
                $( $accumulator )*
                {
                    assert_matches!(
                        ...
                        Some(eyeball_im::VectorDiff::PushBack { value }) => {
                            assert_matches!(
                                **value,
                                ui::timeline::TimelineItemKind::Virtual(
                                    ui::timeline::VirtualTimelineItem::DateDivider(_)
                                ),
                ...
                $( $accumulator )*
                {
                    assert_matches!(
                        ...
                        Some(eyeball_im::VectorDiff::PushBack { value }) => {
                            assert_matches!(
                                &**value,
                                ui::timeline::TimelineItemKind::Event(event_timeline_item) => {
                                    assert_eq!(
                    ...
                    assert_matches!(
                        ...
                        Some(eyeball_im::VectorDiff::PushFront { value }) => {
                            assert_matches!(
                                &**value,
                                ui::timeline::TimelineItemKind::Virtual(
                                    ui::timeline::VirtualTimelineItem::TimelineStart
                                ),
...
async fn create_one_room(
    ...
    stream: &mut Pin<&mut impl Stream<Item = matrix::Result<UpdateSummary>>>,
    room_id: &RoomId,
    room_name: String,
) -> Result<()> {
