...
//
// Ported from tuwunel `src/database/ser.rs`. The CBOR wrapper is not ported,
...
//! use big-endian bytes, and [`Json`] delegates its payload to that
...
use crate::{Error, Result, error::type_name, unhandled};

/// Serializes a value into an owned byte vector.
///
/// The record codec determines the representation. Use [`Json`] to delegate
/// the wrapped payload to that format.
///
/// # Panics
///
    ...
    T: Serialize,
{
    ...
    val.serialize(&mut serializer).map_err(|error| Error::SerdeSer(error.to_string().into()))?;
...
impl<W: Write> Serializer<'_, W> {
    ...
    fn record_start(&mut self) -> Result {
        ...
        if replace(&mut self.sep, true) { self.separator() } else { Ok(()) }
    }
...
impl<W: Write> ser::Serializer for &mut Serializer<'_, W> {
    ...
    fn serialize_i64(self, v: i64) -> Result<Self::Ok> {
        self.write(&v.to_be_bytes()).inspect(|()| self.sep = false)
    }
...
impl<W: Write> ser::SerializeTuple for &mut Serializer<'_, W> {
    ...
    fn serialize_element<T: Serialize + ?Sized>(&mut self, val: &T) -> Result<Self::Ok> {
        self.record_start()?;
        val.serialize(&mut **self).inspect(|()| self.sep = true)
    }
