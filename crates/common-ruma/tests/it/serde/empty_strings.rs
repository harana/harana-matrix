mod string {
    use common_ruma::canonical_json::assert_to_canonical_json_eq;
    ...
    struct StringStruct {
        #[serde(
            default,
            deserialize_with = "common_ruma::serde::empty_string_as_none",
            serialize_with = "common_ruma::serde::none_as_empty_string"
        )]
        x: Option<String>,
    }
...
mod user {
    use common_ruma::{OwnedUserId, canonical_json::assert_to_canonical_json_eq, owned_user_id};
    use serde::{Deserialize, Serialize};
    use serde_json::{from_value as from_json_value, json};
