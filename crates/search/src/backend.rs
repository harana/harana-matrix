...
use ruma::{MilliSecondsSinceUnixEpoch, OwnedEventId, OwnedUserId, RoomId, UInt};
...
pub struct IndexableEvent {
    ...
    ///
    /// Please use the `sdk_common::TimelineEvent::timestamp` as much as
    /// possible as it protects against malformed `origin_server_ts`. At worst,
    /// use the `sdk_common::serde_helpers::extract_timestamp` function.
