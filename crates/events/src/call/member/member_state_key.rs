...
use crate::__ruma::{OwnedUserId, UserId};
use serde::{
...
impl<'de> Deserialize<'de> for CallMemberStateKey {
    ...
    where
        D: Deserializer<'de>,
    {
        let s = crate::__ruma::serde::deserialize_cow_str(deserializer)?;
    ...
    fn from_str(state_key: &str) -> Result<Self, Self::Err> {
        ...
        let Some(colon_idx) = state_key.find(':') else {
            return Err(KeyParseError::InvalidUser {
                ...
                error: crate::__ruma::IdParseError::MissingColon,
            });
        };
...
pub enum KeyParseError {
    ...
    InvalidUser {
        ...
        error: crate::__ruma::IdParseError,
    },
...
mod tests {
    ...
    use crate::__ruma::owned_user_id;
