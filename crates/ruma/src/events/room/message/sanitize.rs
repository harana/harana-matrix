...
pub fn remove_plain_reply_fallback(mut s: &str) -> &str {
    // A reply fallback must begin with a mention of the original sender between `<`
    // and `>`, and emotes add `*` as a prefix. If there is no newline, removing
    // the detected fallback would result in an empty string.
