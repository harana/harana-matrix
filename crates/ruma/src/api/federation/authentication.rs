...
impl XMatrix {
    ...
    pub fn verify_request<T: AsRef<[u8]>>(
        ...
        let mut request_object = Self::request_object(request, &self.origin, destination)
            .map_err(|error| crate::signatures::VerificationError::Json(error.into()))?;
        ...
        Ok(crate::signatures::verify_json(public_key_map, &request_object)?)
    }
}
...
pub enum XMatrixVerificationError {
    ...
    Signature(#[from] crate::signatures::VerificationError),
}
