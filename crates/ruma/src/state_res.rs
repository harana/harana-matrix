...
//! specification.
//!
//! Vendored from `ruma-state-res`, with its cross-crate paths pointing at this
//! crate instead: `ruma_common` is the crate root, `ruma_events` is
//! [`crate::events`], and `ruma_signatures` is [`crate::signatures`].
//!
//! When creating or receiving a PDU (or event), a server must check whether it is valid and how it
//! affects the room state. The purpose of this module is to provide functions that solve that.
//!
...
//! 2. [`crate::signatures::verify_event()`] - The event should be dropped on error. The PDU should
//!    be redacted before checking the authorization rules if the result is `Verified::Signatures`.
...
//! # Event helper types
//!
//! The types in [`crate::events`] use strict deserialization rules according to their definition in
...
//!
//! The types in [`crate::events`] are still appropriate to be used to create a new event, or to
...
//! [state resolution]: https://spec.matrix.org/v1.19/rooms/v11/#state-resolution
