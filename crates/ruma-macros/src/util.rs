...
fn ruma_crate_path() -> TokenStream {
    match crate_name("ruma") {
        ...
        // Downstream crates that only depend on matrix-sdk reach `ruma` through its re-export.
        Err(_) => match crate_name("matrix-sdk") {
            Ok(FoundCrate::Name(name)) => {
                let import = format_ident!("{name}");
                quote! { ::#import::ruma }
            }
...
impl RumaEvents {
    ...
    pub(crate) fn new() -> Self {
        let ruma = ruma_crate_path();
        Self(quote! { #ruma::events })
    }
