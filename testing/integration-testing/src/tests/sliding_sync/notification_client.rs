...
async fn test_notification() -> Result<()> {
    ...
    if let Some(event_id) = invited_room.invite_state.events.iter().find_map(|event| {
        ...
        assert!(matches!(notification.unwrap_err(), Error::SdkError(matrix::Error::Http(..))));
    } else {
        ...
        assert_matches!(
            ...
            NotificationEvent::Timeline(
                ...
                assert_matches!(
                    ...
                    matrix::ruma::events::AnySyncTimelineEvent::MessageLike(
                        matrix::ruma::events::AnySyncMessageLikeEvent::RoomMessage(
                            SyncMessageLikeEvent::Original(event)
                        )
                    ) => {
                        assert_matches!(
                            &event.content.msgtype,
                            matrix::ruma::events::room::message::MessageType::Text(text) => {
