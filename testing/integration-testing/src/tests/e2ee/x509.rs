...
const OID_PKCS9_EMAIL_ADDRESS: &[u64] = &[1, 2, 840, 113549, 1, 9, 1];
...
async fn test_user_is_verified_if_their_msk_is_signed() -> anyhow::Result<()> {
    ...
    let alice_x509_signer: Arc<dyn RawX509Signer> = Arc::new(
...
fn cert_and_key_with_email_signed_by(
    ...
    cert_params
        .distinguished_name
        .push(DnType::CustomDnType(OID_PKCS9_EMAIL_ADDRESS.to_vec()), email);
