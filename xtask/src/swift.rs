...
/// The full SDK, `client-matrix-ffi`.
const FULL_SDK: FfiPackage = FfiPackage {
    crate_name: "client-matrix-ffi",
    library_name: "libclient_matrix_ffi.a",
    features: "sentry",
    framework_name: "MatrixSDKFFI",
};

/// The crypto module on its own, `client-crypto-ffi`, for clients that
/// already depend on another SDK.
const CRYPTO_SDK: FfiPackage = FfiPackage {
    crate_name: "client-crypto-ffi",
    library_name: "libclient_crypto_ffi.a",
    features: "",
...
fn build_library() -> Result<()> {
    ...
    let ffi_directory = root_directory.join("bindings/apple/generated/client_matrix_ffi");
